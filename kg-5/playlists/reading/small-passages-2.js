export default {
  label: 'Small Passages 2',
  id: 'small-passages-2',
  lockAfter: 3,
  list: [
    {
      id: '100',
      type: 'passage',
      label: 'Dolly, my golden fish',
      data: {
        title: 'Dolly, my golden fish',
        text: `Today we will meet Dolly, my golden fish. You will feel happy to see her swim swiftly in her pet jar. My mom presented it for my birthday. I feel so happy to take care of her. I feed her every day in the morning. When I go near the tank, she will swim fast here and there. I know she is happy to see me. My pet knows me. It may be a surprise to you. But, it is true. Come home to see my pet.`
      }
    },
    {
      id: '200',
      type: 'passage',
      label: 'Ski, a sapling',
      data: {
        title: 'Ski, a sapling',
        text: `So soft to touch! Have you touched the tender leaves of a baby plant? I have touched it. Last week, in the science exhibition, I saw a baby plant. Two little leaves in light green color. The stem looked white and tender. I can see the seed with its seed coat under the leaves.  The baby plant swayed in the soft breeze. Hello, baby plant!`
      }
    },
    {
      id: '300',
      type: 'passage',
      label: 'One, two, three',
      data: {
        title: 'One, two, three',
        text: `Little Vino liked to walk up and down on the steps in her school. When she walked, she always counted one, two, three, till she reached the floor. It made her happy. Hop, hop, and hop. With a smile, she will hop down the steps. Luckily, no one noticed her.`
      }
    },
    {
      id: '400',
      type: 'passage',
      label: 'Curve of a moon',
      data: {
        title: 'Curve of a moon',
        text: `A thin curve of the moon looked bright on the night sky. I love watching it from our terrace. My parents are sitting there on the steps and discussing something. I can feel the cool, night breeze. Though it is dark, I am not afraid. The blinking stars in the night sky watch me safely.`
      }
    },
    {
      id: '500',
      type: 'passage',
      label: 'Dress like a doll',
      data: {
        title: 'Dressing the doll',
        text: `I always like to dress my doll. When I am alone at home, I have my doll to play with. My doll has so many tiny dresses. Will you believe it if I tell you I made them? I used my toy scissors to cut the cloth. Then, I used bell pins to join them together. I don’t know how to use a needle.  Every day, I make her wear a new dress. Do you have a doll?`
      }
    },
    {
      id: '600',
      type: 'passage',
      label: 'Drops of water',
      data: {
        title: 'Drops of water',
        text: `Each time you feel thirsty, grab your water bottle and take a sip. This will make you feel better. Look at the birds in the sky. Where will they go when they feel thirsty? They search for a place with water. Unfortunately, they usually find unhealthy water. Who polluted it? Think! Every living thing in this world has the right to live. Leave a bowl of water on your terrace. Let birds quench their thirst. You will be blessed!`
      }
    },
    {
      id: '700',
      type: 'passage',
      label: 'Dews on leaves',
      data: {
        title: 'Dews on leaves',
        text: `Look at the pot of plant near the kitchen window. The leaves are broad and dark green. Little, little drops of water! The water drops reflect prism colours. It is so chilly in the morning! There are dew-drops on the leaves. Can you tell me what colors are there? I can see shades of red and green. What colors do you see?`
      }
    },
    {
      id: '800',
      type: 'passage',
      label: 'Sundays are holidays',
      data: {
        title: 'Sundays are holidays',
        text: `Every week we have Sunday as our holiday. Why Sunday is so special for me? My father takes me along for his morning walk. We walk briskly to the seashore. It is just ten minutes from my home. I can hear the rolling waves of the sea. It is so good to watch the waves in the morning sun rays.`
      }
    },
    {
      id: '900',
      type: 'passage',
      label: 'A Crab tale',
      data: {
        title: 'A Crab tale',
        text: `It is raining heavily today. I love looking at the rain. Suddenly, I see something crawling in the water on the street. It is a Crab. Crabs are wonderful creatures to watch. I wish I could catch it. But, I am afraid of its pincers. They may hurt me.`
      }
    },
    {
      id: '1000',
      type: 'passage',
      label: 'Cycle my way',
      data: {
        title: 'Cycle my way',
        text: `Arun likes to go for a ride in his cycle to the park near his home. Every day, after he comes back from school, he rides with his friends to the park. The park has a separate track for bicycle rides. It is safe to ride in the park. Cycling is a very good exercise. Not only little boys, but even little girls also come there for cycling.`
      }
    }
  ]
};
