export default {
  label: 'ध्वनि',
  id: 'hi-sound-2',
  img: 'sound',
  list: [
    {
      type: 'slides',
      id: 'animals',
      label: 'जानवर',
      commonData: {
        audio: 'hi/hi-animals-2.mp3'
      },
      data: [
        {
          audioOffset: 0,
          text: `kangaroo|कंगेरू
          leopard|तेंदुआ
          mongoose|नेवला
          mosquito|मच्छर
          wolf|भेड़िया
          cockroach|तिलचट्टा
          garden-lizard|उद्यान छिपकली
          lizard|छिपकली
          rabbit|खरगोश
          alligator|घड़ियाल`
        },
        {
          audioOffset: 20,
          text: `boar|जंगली सूअर
          cricket| झिंगुर
          hyena|लकड़बग्धा
          polarbear| ध्रुवीय भालू
          antelope|बारहसिंगा
          bug| खटमल
          jackal| सियार
          prawn|  झींगे
          baboon| लंगूर
          bull| सांड`
        },
        {
          audioOffset: 40,
          text: `python|अजगर
          bison|जंगली सांड
          chameleon|गिरगिट
          porcupine|साही
          bee|मधुमक्खी
          goldfish|सुनहरी मछली
          seahorse|समुद्री घोड़े
          mole|छछूँदर
          spider|मकड़ी
          caterpillar|इल्ली
          earthworm|केंचुआ`
        }
      ]
    },
    {
      type: 'slides',
      id: 'birds',
      label: 'चिड़िया',
      data: {
        audio: 'hi/hi-birds-vegetables-2.mp3',
        audioOffset: 0,
        text: `woodpecker|कठफोड़वा
          cuckoo|कोयल
          pheasant|तीतर
          falcon|बाज़
          swan| हंस
          bulbul|बुलबुल
          spotted-dove| चित्तीदार कबूतर
          seagull|सामुद्रिक चिड़िया
          hawk|बाज`
      }
    },
    {
      type: 'slides',
      id: 'vegetables',
      label: 'सब्जि',
      commonData: {
        audio: 'hi/hi-birds-vegetables-2.mp3'
      },
      data: [
        {
          audioOffset: 18,
          text: `corianderleaf| धनिया पत्ता
          curryleaf| करी पत्ता
          drumstick| सेहजन
          greenpeas| मटर
          ladies-finger| भिन्डी
          mintleaf| पुदीना
          peanut| मूंगफली
          radish| मूली
          spinach| पालक
          chickpea|चना`
        },
        {
          audioOffset: 38,
          text: `mustard-greens|सरसों का पत्ता
          turmeric|हल्दी
          turnip| शलगम
          fenugreek| हरी मेथी
          tamarind|इमली
          ashgourd|रखिया
          broad-beans|सेम फली
          snake-gourd|चिचिण्डा
          cluster-beans|गँवार फली
          kohlrabi|बन्द-गोभी`
        },
        {
          audioOffset: 58,
          text: `tulsi|तुलसी
          bottlegourd|लौकी
          ivygourd|कुंदरू
          ridgegourd|तुरई 
          vegetables|सब्जियां`
        }
      ]
    },
    {
      type: 'slides',
      id: 'flowers',
      label: 'पुष्प',
      commonData: {
        audio: 'hi/hi-flower-vehicle-school-2.mp3'
      },
      data: [
        {
          audioOffset: 0,
          text: `rose | गुलाब
          lotus | कमल
          sunflower | सूर्यमुखी
          marigold | गेंदा
          hibiscus | गुडहल
          mogra| मोगरा
          daffodil| नरगिस
          gulmohar|गुलमोहर
          lily|कुमुदनी
          pansy|बनफूल`
        },
        {
          audioOffset: 20,
          text: `poppy|खसखस
          daisy|गुलबहार
          magnolia|चम्पा
          periwinkle| सदाबहार
          water-lily|नीलकमल`
        }
      ]
    },
    {
      type: 'slides',
      id: 'vehicles',
      label: 'वाहन',
      commonData: {
        audio: 'hi/hi-flower-vehicle-school-2.mp3'
      },
      data: [
        {
          audioOffset: 30,
          text: `aeroplane| हवाईजहाज
          boat| नाव
          bus| बस
          car| गाड़ी
          cycle| साइकिल
          helicoptor| घिरनीदार विमान
          ship| समुंद्रीजहाज
          tractor| ट्रैक्टर
          train| रेलगाडी
          ambulance| रोगीवाहन`
        },
        {
          audioOffset: 50,
          text: `auto| रिक्शा
          cart| हाथगाडी
          excavator| खोदक मशीन
          fire-engine| दमकल
          lorry| लारी`
        }
      ]
    },
    {
      type: 'slides',
      id: 'school',
      label: 'विद्यालय',
      commonData: {
        audio: 'hi/hi-flower-vehicle-school-2.mp3'
      },
      data: [
        {
          audioOffset: 60,
          text: `eraser| रबड़
          pen| कलम
          pencil| पेंसिल
          sharpener| चोखा करने वाला
          ruler| पेमाना
          schoolbag| बस्ता`
        },
        {
          audioOffset: 72,
          text: `scissors| कैंची
          waterbottle| पानी की बोतल
          calculator| गणना यंत्र
          book| किताब
          color-pencils | रंग पेंसिल
          crayons|चित्रांकनी
          ink|स्याही`
        }
      ]
    },
    {
      type: 'slides',
      id: 'tools',
      label: 'उपकरण',
      commonData: {
        audio: 'hi/hi-tools-jobs-2.mp3'
      },
      data: [
        {
          audioOffset: 0,
          text: `axe| कुल्हाडी
          balance| तराज़ू
          blackboard | श्यामपट
          hose | रबर का पाइप
          microscope| सूक्ष्मदर्शी
          needle | सुई
          oven | कुठला
          plier| प्‍लास
          plough | हल
          saw| आरा`
        },
        {
          audioOffset: 20,
          text: `spade| कुदाल
          stethoscope |परिश्रावक
          telescope| दूरबीन
          test-tube | परखनली
          thermometer| तापमान-यंत्र
          trowel| करनी
          awl | सूआ
          compass| दिशा सूचक यंत्र
          easel| चित्रफलक
          hacksaw| लोहा-आरी`
        },
        {
          audioOffset: 40,
          text: `scalpel| छुरी
          screw|पेंच
          screwdriver|पेंचकस
          flute|बांसुरी
          tabla|तबला`
        }
      ]
    },
    {
      type: 'slides',
      id: 'jobs',
      label: 'नौकरियां',
      commonData: {
        audio: 'hi/hi-tools-jobs-2.mp3'
      },
      data: [
        {
          audioOffset: 50,
          text: `astronaut|अंतरिक्ष यात्री
          fisherman|मछुआरा
          postman|डाकिया
          teacher|शिक्षक
          blacksmith|लोहार
          cobbler|मोची
          goldsmith|सुनार
          potter|कुम्हार
          weaver|जुलाहा
          carpenter|बढ़ई`
        },
        {
          audioOffset: 70,
          text: `nurse|दाई
          chef|बावर्ची
          doctor|चिकित्सक
          soldier|सैनिक
          barber|नाई
          washerman|धोबी`
        }
      ]
    },
    {
      type: 'slides',
      id: 'other-images',
      label: 'अन्य छवियां',
      commonData: {
        audio: 'hi/hi-other-2.mp3'
      },
      data: [
        {
          audioOffset: 0,
          text: `bullock-cart|बैलगाड़ी
          warrior|योद्धा
          knight|क्षत्रिय
          chariot|रथ
          park|बाग
          garden|उद्यान 
          horse-carriage|घोड़ागाड़ी
          touch-me-not|छुई मुई
          yagna|यज्ञ
          yarn-ball |ऊन`
        },
        {
          audioOffset: 20,
          text: `bow| धनुष 
          arrow| बाण
          pouch|थैली
          top|लट्टू 
          hexagon|षटकोण
          trident|त्रिशूल
          damaru|डमरू
          stick|डंडा
          twig| टहनी
          dhol|ढोल`
        },
        {
          audioOffset: 40,
          text: `palanquin|पालकी
          medicine|दवा
          balloon|गुब्बारा
          box|डिब्बा
          coin|सिक्का`
        }
      ]
    },
    {
      type: 'rightOne',
      label: 'चित्र चुनें',
      lockAfter: 3,
      id: 'pick-word',
      commonData: {
        title: 'ध्वनि सुनकर चित्र का चयन करें',
        type: 'image'
      },
      data: [
        {
          audio: 'hi/hi-animals-2.mp3',
          words:
            'kangaroo, leopard, mongoose, mosquito, wolf, cockroach, garden-lizard, lizard, rabbit, alligator'
        },
        {
          audio: 'hi/hi-animals-2.mp3',
          audioOffset: 20,
          words:
            'boar, cricket, hyena, polarbear, antelope, bug, jackal, prawn, baboon, bull'
        },
        {
          audio: 'hi/hi-animals-2.mp3',
          audioOffset: 40,
          words:
            'python, bison, chameleon, porcupine, bee, goldfish, seahorse, mole, spider, caterpillar, earthworm'
        },
        {
          audio: 'hi/hi-birds-vegetables-2.mp3',
          audioOffset: 0,
          words:
            'woodpecker, cuckoo, pheasant, falcon, swan, bulbul, spotted-dove, seagull, hawk'
        },
        {
          audio: 'hi/hi-birds-vegetables-2.mp3',
          audioOffset: 18,
          words:
            'corianderleaf, curryleaf, drumstick, greenpeas, ladies-finger, mintleaf, peanut, radish, spinach, chickpea'
        },
        {
          audio: 'hi/hi-birds-vegetables-2.mp3',
          audioOffset: 38,
          words:
            'mustard-greens, turmeric, turnip, fenugreek, tamarind, ashgourd, broad-beans, snake-gourd, cluster-beans, kohlrabi'
        },
        {
          audio: 'hi/hi-birds-vegetables-2.mp3',
          audioOffset: 58,
          words: 'tulsi, bottlegourd, ivygourd, ridgegourd, vegetables'
        },
        {
          audio: 'hi/hi-flower-vehicle-school-2.mp3',
          audioOffset: 0,
          words:
            'rose, lotus, sunflower, marigold, hibiscus, mogra, daffodil, gulmohar, lily, pansy'
        },
        {
          audio: 'hi/hi-flower-vehicle-school-2.mp3',
          audioOffset: 20,
          words: 'poppy, daisy, magnolia, periwinkle, water-lily'
        },
        {
          audio: 'hi/hi-flower-vehicle-school-2.mp3',
          audioOffset: 30,
          words:
            'aeroplane, boat, bus, car, cycle, helicoptor, ship, tractor, train, ambulance'
        },
        {
          audio: 'hi/hi-flower-vehicle-school-2.mp3',
          audioOffset: 50,
          words: 'auto, cart, excavator, fire-engine, lorry'
        },
        {
          audio: 'hi/hi-flower-vehicle-school-2.mp3',
          audioOffset: 60,
          words: 'eraser, pen, pencil, sharpener, ruler, schoolbag'
        },
        {
          audio: 'hi/hi-flower-vehicle-school-2.mp3',
          audioOffset: 72,
          words:
            'scissors, waterbottle, calculator, book, color-pencils, crayons, ink'
        },
        {
          audio: 'hi/hi-tools-jobs-2.mp3',
          audioOffset: 0,
          words:
            'axe, balance, blackboard, hose, microscope, needle, oven, plier, plough, saw'
        },
        {
          audio: 'hi/hi-tools-jobs-2.mp3',
          audioOffset: 20,
          words:
            'spade, stethoscope, telescope, test-tube, thermometer, trowel, awl, compass, easel, hacksaw'
        },
        {
          audio: 'hi/hi-tools-jobs-2.mp3',
          audioOffset: 40,
          words: 'scalpel, screw, screwdriver, flute, tabla'
        },
        {
          audio: 'hi/hi-tools-jobs-2.mp3',
          audioOffset: 50,
          words:
            'astronaut, fisherman, postman, teacher, blacksmith, cobbler, goldsmith, potter, weaver, carpenter'
        },
        {
          audio: 'hi/hi-tools-jobs-2.mp3',
          audioOffset: 70,
          words: 'nurse, chef, doctor, soldier, barber, washerman'
        },
        {
          audio: 'hi/hi-other-2.mp3',
          audioOffset: 0,
          words:
            'bullock-cart, warrior, knight, chariot, park, garden, horse-carriage, touch-me-not, yagna, yarn-ball'
        },
        {
          audio: 'hi/hi-other-2.mp3',
          audioOffset: 20,
          words:
            'bow, arrow, pouch, top, hexagon, trident, damaru, stick, twig, dhol'
        },
        {
          audio: 'hi/hi-other-2.mp3',
          audioOffset: 40,
          words: 'palanquin, medicine, balloon, box, coin'
        }
      ]
    }
  ]
};
