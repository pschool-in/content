export default {
  label: 'Word Meanings',
  id: 'word-meaning-7',
  lockAfter: 2,
  list: [
    {
      id: 'word-meanings-1',
      type: 'slides2',
      label: 'Word Meanings',
      commonData: {
        title: 'Word Meanings',
        displayType: 'custom',
        autoPlay: false,
        substep: false
      },
      data: [
        {
          steps: [
            [
              {
                type: 'title',
                text: 'abandon'
              },
              '- forsake; leave behind',
              {
                type: 'html',
                text: 'Eg: <br><br> When the ship was sinking, the crew had to <i><b>abandon</b></i> it and escape in lifeboats.<br><br> He decided to <i><b>abandon</b></i> his old car and buy a new one.'
              }
            ],
            [
              {
                type: 'title',
                text: 'abundant'
              },
              '- present in great quantity',
              {
                type: 'html',
                text: 'Eg: <br><br> The forest is home to an <i><b>abundant</b></i> variety of wildlife.<br><br> After a rainy season, the fields are filled with <i><b>abundant</b></i> crops.'
              }
            ],
            [
              {
                type: 'title',
                text: 'access'
              },
              '- the right to enter',
              {
                type: 'html',
                text: 'Eg: <br><br> The keycard provides <i><b>access</b></i> to the secure building.<br><br> The hiker had to find a way to gain <i><b>access</b></i> to the remote mountain region.'
              }
            ],
            [
              {
                type: 'title',
                text: 'accommodate'
              },
              '- have room for; hold without crowding',
              {
                type: 'html',
                text: 'Eg: <br><br> The spacious hall can <i><b>accommodate</b></i> a large audience for events.<br><br> The hotel was able to <i><b>accommodate</b></i> all the guests comfortably.'
              }
            ],
            [
              {
                type: 'title',
                text: 'accumulate'
              },
              '- get or gather together',
              {
                type: 'html',
                text: "Eg: <br><br> Over the years, he managed to <i><b>accumulate</b></i> a valuable collection of antique books.<br><br> It's essential to <i><b>accumulate</b></i> evidence before making a decision."
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'adapt'
              },
              '- make fit for, or change to suit a new purpose',
              {
                type: 'html',
                text: 'Eg: <br><br> Some animals can <i><b>adapt</b></i> to different environments by changing their behavior.<br><br> You need to <i><b>adapt</b></i> to the new software to use it effectively.'
              }
            ],
            [
              {
                type: 'title',
                text: 'adhere'
              },
              '- stick to firmly',
              {
                type: 'html',
                text: "Eg: <br><br> The glue will <i><b>adhere</b></i> the two pieces of wood together.<br><br> It's important to <i><b>adhere</b></i> to a healthy lifestyle for better well-being."
              }
            ],
            [
              {
                type: 'title',
                text: 'agony'
              },
              '- intense feelings of suffering; acute mental or physical pain',
              {
                type: 'html',
                text: 'Eg: <br><br> The patient was in <i><b>agony</b></i> due to the severe injury.<br><br> Losing a loved one can cause immense <i><b>agony</b></i> and grief.'
              }
            ],
            [
              {
                type: 'title',
                text: 'allegiance'
              },
              '- the act of binding yourself to a course of action',
              {
                type: 'html',
                text: 'Eg: <br><br> Pledging <i><b>allegiance</b></i> to the flag is a common practice in many countries.<br><br> The soldiers swore their <i><b>allegiance</b></i> to the nation.'
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'ambition'
              },
              '- a strong drive for success',
              {
                type: 'html',
                text: 'Eg: <br><br> Her <i><b>ambition</b></i> is to become a world-renowned scientist.<br><br> He pursued his <i><b>ambition</b></i> to become a successful entrepreneur.'
              }
            ],
            [
              {
                type: 'title',
                text: 'ample'
              },
              '- more than enough in size or scope or capacity',
              {
                type: 'html',
                text: 'Eg: <br><br> We have <i><b>ample</b></i> time to complete the project ahead of the deadline.<br><br> The spacious living room provides <i><b>ample</b></i> space for gatherings.'
              }
            ],
            [
              {
                type: 'title',
                text: 'anguish'
              },
              '- extreme distress of body or mind',
              {
                type: 'html',
                text: 'Eg: <br><br> The news of the accident caused great <i><b>anguish</b></i> to the family.<br><br> The thought of losing her pet filled her with <i><b>anguish</b></i>.'
              }
            ],
            [
              {
                type: 'title',
                text: 'anticipate'
              },
              '- regard something as probable or likely',
              {
                type: 'html',
                text: 'Eg: <br><br> We can <i><b>anticipate</b></i> a warm welcome from our friends when we visit.<br><br> Experts <i><b>anticipate</b></i> an increase in the stock market.'
              }
            ],
            [
              {
                type: 'title',
                text: 'anxious'
              },
              '- causing or fraught with or showing nervousness',
              {
                type: 'html',
                text: 'Eg: <br><br> Waiting for exam results can be an <i><b>anxious</b></i> time for students.<br><br> The upcoming presentation made her feel <i><b>anxious</b></i>.'
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'apparel'
              },
              '- clothing in general',
              {
                type: 'html',
                text: 'Eg: <br><br> The store offers a wide range of <i><b>apparel</b></i> for all seasons.<br><br> Traditional <i><b>apparel</b></i> varies from culture to culture.'
              }
            ],
            [
              {
                type: 'title',
                text: 'appeal'
              },
              '- earnest or urgent request',
              {
                type: 'html',
                text: 'Eg: <br><br> The charity made an <i><b>appeal</b></i> for donations to help those in need.<br><br> The lawyer presented a strong <i><b>appeal</b></i> to the judge.'
              }
            ],
            [
              {
                type: 'title',
                text: 'apprehensive'
              },
              '- in fear or dread of possible evil or harm',
              {
                type: 'html',
                text: 'Eg: <br><br> He felt <i><b>apprehensive</b></i> about the outcome of the medical tests.<br><br> The stormy weather made passengers <i><b>apprehensive</b></i> about their flight.'
              }
            ],
            [
              {
                type: 'title',
                text: 'arid'
              },
              '- lacking sufficient water or rainfall',
              {
                type: 'html',
                text: 'Eg: <br><br> The desert is known for its <i><b>arid</b></i> climate with very little rain.<br><br> The <i><b>arid</b></i> land made agriculture challenging.'
              }
            ],
            [
              {
                type: 'title',
                text: 'arrogant'
              },
              '- having or showing feelings of unwarranted importance',
              {
                type: 'html',
                text: 'Eg: <br><br> His <i><b>arrogant</b></i> behavior alienated his colleagues.<br><br> She is known for her <i><b>arrogant</b></i> attitude toward others.'
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'awe'
              },
              '- an overwhelming feeling of wonder or admiration',
              {
                type: 'html',
                text: 'Eg: <br><br> The breathtaking natural beauty of the Grand Canyon fills visitors with <i><b>awe</b></i>.<br><br> His extraordinary talent as a musician inspired <i><b>awe</b></i> among his peers.'
              }
            ],
            [
              {
                type: 'title',
                text: 'barren'
              },
              '- completely wanting or lacking',
              {
                type: 'html',
                text: 'Eg: <br><br> The desert landscape is <i><b>barren</b></i>, with little vegetation.<br><br> The <i><b>barren</b></i> field needed fertile soil for crops to grow.'
              }
            ],
            [
              {
                type: 'title',
                text: 'beacon'
              },
              '- a fire that can be seen from a distance',
              {
                type: 'html',
                text: 'Eg: <br><br> The lighthouse serves as a maritime <i><b>beacon</b></i> to guide ships safely to the shore.<br><br> The campfire acted as a <i><b>beacon</b></i> in the darkness.'
              }
            ],
            [
              {
                type: 'title',
                text: 'beneficial'
              },
              '- promoting or enhancing well-being',
              {
                type: 'html',
                text: 'Eg: <br><br> Regular exercise is <i><b>beneficial</b></i> for your health.<br><br> The new policy had a <i><b>beneficial</b></i> impact on the environment.'
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'blunder'
              },
              '- an embarrassing mistake',
              {
                type: 'html',
                text: 'Eg: <br><br> Making that public statement was a major <i><b>blunder</b></i> for the politician.<br><br> She realized her <i><b>blunder</b></i> after sending the wrong email.'
              }
            ],
            [
              {
                type: 'title',
                text: 'boisterous'
              },
              '- marked by exuberance and high spirits',
              {
                type: 'html',
                text: 'Eg: <br><br> The children were <i><b>boisterous</b></i> at the playground, laughing and playing games.<br><br> The party became <i><b>boisterous</b></i> as the night went on.'
              }
            ],
            [
              {
                type: 'title',
                text: 'boycott'
              },
              '- refusal to have commercial dealings with some organization',
              {
                type: 'html',
                text: "Eg: <br><br> The consumers decided to <i><b>boycott</b></i> the company's products due to ethical concerns.<br><br> The <i><b>boycott</b></i> of the event was a statement against unfair practices."
              }
            ],
            [
              {
                type: 'title',
                text: 'burden'
              },
              '- weight to be carried or borne',
              {
                type: 'html',
                text: 'Eg: <br><br> The heavy backpack was a significant <i><b>burden</b></i> for the hiker.<br><br> Taking care of a sick family member can be an emotional <i><b>burden</b></i>.'
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'campaign'
              },
              '- related operations aimed at achieving a particular goal',
              {
                type: 'html',
                text: "Eg: <br><br> The political <i><b>campaign</b></i> included rallies, speeches, and advertisements to win votes.<br><br> The charity's <i><b>campaign</b></i> raised funds for a good cause."
              }
            ],
            [
              {
                type: 'title',
                text: 'capacity'
              },
              '- capability to perform or produce',
              {
                type: 'html',
                text: 'Eg: <br><br> The factory has the <i><b>capacity</b></i> to produce thousands of units in a day.<br><br> She has the <i><b>capacity</b></i> to learn new languages quickly.'
              }
            ],
            [
              {
                type: 'title',
                text: 'capital'
              },
              '- a large alphabetic character used in writing or printing',
              {
                type: 'html',
                text: "Eg: <br><br> In the sentence, the word 'Apple' begins with a <i><b>capital</b></i> 'A'.<br><br> When writing a name, it's common to use a <i><b>capital</b></i> letter for the first letter."
              }
            ],
            [
              {
                type: 'title',
                text: 'chronological'
              },
              '- relating to or arranged according to the order of time',
              {
                type: 'html',
                text: "Eg: <br><br> The history book presents events in a <i><b>chronological</b></i> order, starting with the earliest.<br><br> The timeline provides a <i><b>chronological</b></i> view of the project's milestones."
              }
            ],
            [
              {
                type: 'title',
                text: 'civic'
              },
              '- of or relating or belonging to a city',
              {
                type: 'html',
                text: "Eg: <br><br> <i><b>Civic</b></i> organizations often work to improve the well-being of the local community.<br><br> The mayor's duties include addressing <i><b>civic</b></i> matters and city development."
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'clarity'
              },
              '- the quality of being coherent and easily understood',
              {
                type: 'html',
                text: "Eg: <br><br> The instructor's explanation provided <i><b>clarity</b></i> on the complex topic.<br><br> A good presentation should strive for <i><b>clarity</b></i> to ensure the audience understands the content."
              }
            ],
            [
              {
                type: 'title',
                text: 'collaborate'
              },
              '- work together on a common enterprise or project',
              {
                type: 'html',
                text: 'Eg: <br><br> The two companies decided to <i><b>collaborate</b></i> on a research project.<br><br> Team members need to <i><b>collaborate</b></i> effectively to achieve project goals.'
              }
            ],
            [
              {
                type: 'title',
                text: 'collide'
              },
              '- crash together with violent impact',
              {
                type: 'html',
                text: "Eg: <br><br> The two cars <i><b>collided</b></i> at the intersection, causing significant damage.<br><br> In a high-speed chase, the police car <i><b>collided</b></i> with the suspect's vehicle."
              }
            ],
            [
              {
                type: 'title',
                text: 'commend'
              },
              '- present as worthy of regard, kindness, or confidence',
              {
                type: 'html',
                text: "Eg: <br><br> The teacher <i><b>commended</b></i> the student's hard work and dedication.<br><br> The supervisor <i><b>commended</b></i> the team for their exceptional performance."
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'commentary'
              },
              '- a written explanation or criticism or illustration',
              {
                type: 'html',
                text: "Eg: <br><br> The sports broadcast includes <i><b>commentary</b></i> from experts who analyze the game.<br><br> The book provides valuable <i><b>commentary</b></i> on the author's life and works."
              }
            ],
            [
              {
                type: 'title',
                text: 'compact'
              },
              '- closely and firmly united or packed together',
              {
                type: 'html',
                text: 'Eg: <br><br> The students formed a <i><b>compact</b></i> group to work on the project.<br><br> The luggage was <i><b>compact</b></i> and easy to carry.'
              }
            ],
            [
              {
                type: 'title',
                text: 'composure'
              },
              '- steadiness of mind under stress',
              {
                type: 'html',
                text: 'Eg: <br><br> Even in challenging situations, she maintained her <i><b>composure</b></i> and stayed calm.<br><br> The athlete displayed remarkable <i><b>composure</b></i> during the high-pressure match.'
              }
            ],
            [
              {
                type: 'title',
                text: 'concise'
              },
              '- expressing much in few words',
              {
                type: 'html',
                text: 'Eg: <br><br> A good summary is <i><b>concise</b></i> and to the point, capturing the main ideas.<br><br> His <i><b>concise</b></i> speech left a strong impact on the audience.'
              }
            ],
            [
              {
                type: 'title',
                text: 'consent'
              },
              '- give an affirmative reply to; respond favorably to',
              {
                type: 'html',
                text: "Eg: <br><br> She needed her parents' <i><b>consent</b></i> to go on the school trip.<br><br> He eagerly gave his <i><b>consent</b></i> to the proposal."
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'consequence'
              },
              '- a phenomenon that is caused by some previous phenomenon',
              {
                type: 'html',
                text: 'Eg: <br><br> The <i><b>consequence</b></i> of not studying for the exam was a low grade.<br><br> Environmental pollution has serious <i><b>consequences</b></i> for our planet.'
              }
            ],
            [
              {
                type: 'title',
                text: 'conserve'
              },
              '- keep in safety and protect from harm, loss, or destruction',
              {
                type: 'html',
                text: "Eg: <br><br> It's important to <i><b>conserve</b></i> our natural resources for future generations.<br><br> The museum works diligently to <i><b>conserve</b></i> valuable artworks."
              }
            ],
            [
              {
                type: 'title',
                text: 'conspicuous'
              },
              '- obvious to the eye or mind',
              {
                type: 'html',
                text: 'Eg: <br><br> Her bright red hat was quite <i><b>conspicuous</b></i> in the crowd.<br><br> The tall mountain peak is <i><b>conspicuous</b></i> from a distance.'
              }
            ],
            [
              {
                type: 'title',
                text: 'constant'
              },
              '- uninterrupted in time and indefinitely long continuing',
              {
                type: 'html',
                text: 'Eg: <br><br> The sound of the waves crashing is a <i><b>constant</b></i> presence at the beach.<br><br> His <i><b>constant</b></i> dedication to his work led to success.'
              }
            ],
            [
              {
                type: 'title',
                text: 'contaminate'
              },
              '- make impure',
              {
                type: 'html',
                text: "Eg: <br><br> Pollution can <i><b>contaminate</b></i> the air and water, causing health problems.<br><br> It's essential to avoid activities that may <i><b>contaminate</b></i> the environment."
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'context'
              },
              '- the set of facts or circumstances that surround a situation',
              {
                type: 'html',
                text: "Eg: <br><br> To understand the meaning of a word, it's important to consider the <i><b>context</b></i> in which it is used.<br><br> The <i><b>context</b></i> of the story provided insight into the character's actions."
              }
            ],
            [
              {
                type: 'title',
                text: 'continuous'
              },
              '- moving in time or space without interruption',
              {
                type: 'html',
                text: "Eg: <br><br> The river flows <i><b>continuous</b></i> from the mountains to the sea.<br><br> The film's <i><b>continuous</b></i> action kept the audience engaged."
              }
            ],
            [
              {
                type: 'title',
                text: 'controversy'
              },
              '- a dispute where there is strong disagreement',
              {
                type: 'html',
                text: 'Eg: <br><br> The <i><b>controversy</b></i> over the new law led to heated debates.<br><br> The <i><b>controversy</b></i> surrounding the scientific discovery was widely discussed.'
              }
            ],
            [
              {
                type: 'title',
                text: 'convenient'
              },
              '- suited to your comfort or purpose or needs',
              {
                type: 'html',
                text: "Eg: <br><br> The location of the hotel is <i><b>convenient</b></i> for tourists, as it's close to major attractions.<br><br> Having a smartphone is <i><b>convenient</b></i> for staying connected on the go."
              }
            ],
            [
              {
                type: 'title',
                text: 'cope'
              },
              '- come to terms with',
              {
                type: 'html',
                text: 'Eg: <br><br> She had to <i><b>cope</b></i> with the loss of her beloved pet.<br><br> Learning to <i><b>cope</b></i> with stress is important for mental well-being.'
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'cordial'
              },
              '- politely warm and friendly',
              {
                type: 'html',
                text: 'Eg: <br><br> The host extended a <i><b>cordial</b></i> welcome to the guests at the party.<br><br> Despite their differences, they maintained a <i><b>cordial</b></i> relationship.'
              }
            ],
            [
              {
                type: 'title',
                text: 'cultivate'
              },
              '- adapt something wild to the environment',
              {
                type: 'html',
                text: 'Eg: <br><br> Farmers <i><b>cultivate</b></i> crops to ensure a successful harvest.<br><br> Gardeners <i><b>cultivate</b></i> plants to thrive in specific conditions.'
              }
            ],
            [
              {
                type: 'title',
                text: 'cumulative'
              },
              '- increasing by successive addition',
              {
                type: 'html',
                text: "Eg: <br><br> Savings can grow <i><b>cumulative</b></i> over time with interest.<br><br> The project's success was a result of the <i><b>cumulative</b></i> efforts of the team."
              }
            ],
            [
              {
                type: 'title',
                text: 'declare'
              },
              '- state emphatically and authoritatively',
              {
                type: 'html',
                text: 'Eg: <br><br> The president will <i><b>declare</b></i> the new policy in his speech tomorrow.<br><br> The witness had to <i><b>declare</b></i> the truth under oath.'
              }
            ],
            [
              {
                type: 'title',
                text: 'deluge'
              },
              '- a heavy rain',
              {
                type: 'html',
                text: 'Eg: <br><br> The sudden <i><b>deluge</b></i> caused flooding in the city streets.<br><br> We had to postpone the picnic due to the heavy <i><b>deluge</b></i>.'
              }
            ]
          ]
        }
      ]
    },
    {
      label: 'Fill in the blanks',
      type: 'matchByDragDrop',
      id: 'drag-drop-3',
      lockAfter: 1,
      commonData: {
        title:
          'Drag the correct option in each blank to form meaningfull sentences.'
      },
      data: [
        `When the ship was sinking, the crew had to *abandon* it and escape in lifeboats.
The forest is home to an *abundant* variety of wildlife.
The keycard provides *access* to the secure building.
The spacious hall can *accommodate* a large audience for events.
Over the years, he managed to *accumulate* a valuable collection of antique books.`,

        `Some animals can *adapt* to different environments by changing their behavior.
The glue will *adhere* the two pieces of wood together.
The patient was in *agony* due to the severe injury.
Pledging *allegiance* to the flag is a common practice in many countries.
Her *ambition* is to become a world-renowned scientist.`,

        `We have *ample* time to complete the project ahead of the deadline.
The news of the accident caused great *anguish* to the family.
We can *anticipate* a warm welcome from our friends when we visit.
Waiting for exam results can be an *anxious* time for students.
The store offers a wide range of *apparel* for all seasons.`,

        `The charity made an *appeal* for donations to help those in need.
He felt *apprehensive* about the outcome of the medical tests.
The desert is known for its *arid* climate with very little rain.
His *arrogant* behavior alienated his colleagues.
The breathtaking natural beauty of the Grand Canyon fills visitors with *awe*.`,

        `The desert landscape is *barren*, with little vegetation.
The lighthouse serves as a maritime *beacon* to guide ships safely to the shore.
Regular exercise is *beneficial* for your health.
Making that public statement was a major *blunder* for the politician.
The children were *boisterous* at the playground, laughing and playing games.`,

        `The consumers decided to *boycott* the company's products due to ethical concerns.
The heavy backpack was a significant *burden* for the hiker.
The political *campaign* included rallies, speeches, and advertisements to win votes.
The factory has the *capacity* to produce thousands of units in a day.
In the sentence, the word 'Apple' begins with a *capital* 'A'.`,

        `The history book presents events in a *chronological* order, starting with the earliest.
Civic organizations often work to improve the well-being of the local community.
The instructor's explanation provided *clarity* on the complex topic.
The two companies decided to *collaborate* on a research project.
The two cars *collided* at the intersection, causing significant damage.`,

        `The teacher *commended* the student's hard work and dedication.
The sports broadcast includes *commentary* from experts who analyze the game.
The students formed a *compact* group to work on the project.
Even in challenging situations, she maintained her *composure* and stayed calm.
A good summary is *concise* and to the point, capturing the main ideas.`,

        `She needed her parents' *consent* to go on the school trip.
The *consequence* of not studying for the exam was a low grade.
It's important to *conserve* our natural resources for future generations.
Her bright red hat was quite *conspicuous* in the crowd.
The sound of the waves crashing is a *constant* presence at the beach.`,

        `Pollution can *contaminate* the air and water, causing health problems.
To understand the meaning of a word, it's important to consider the *context* in which it is used.
The river flows *continuous* from the mountains to the sea.
The *controversy* over the new law led to heated debates.
The location of the hotel is *convenient* for tourists, as it's close to major attractions.`,

        `She had to *cope* with the loss of her beloved pet.
The host extended a *cordial* welcome to the guests at the party.
Farmers *cultivate* crops to ensure a successful harvest.
Savings can grow *cumulative* over time with interest.
He *declared* his intention to run for president.
The *deluge* of rain flooded the streets.`
      ]
    },
    {
      type: 'completeWord',
      id: 'complete-word-3',
      label: 'Complete Word',
      data: [
        `Forsake; leave behind | ABANDON
Present in great quantity | ABUNDANT
The right to enter | ACCESS
Have room for; hold without crowding | ACCOMMODATE
Get or gather together | ACCUMULATE
Make fit for, or change to suit a new purpose | ADAPT
Stick to firmly | ADHERE
Intense feelings of suffering; acute mental or physical pain | AGONY
The act of binding yourself to a course of action | ALLEGIANCE
A strong drive for success | AMBITION`,

        `More than enough in size or scope or capacity | AMPLE
Extreme distress of body or mind | ANGUISH
Regard something as probable or likely | ANTICIPATE
Causing or fraught with or showing nervousness | ANXIOUS
Clothing in general | APPAREL
Earnest or urgent request | APPEAL
In fear or dread of possible evil or harm | APPREHENSIVE
Lacking sufficient water or rainfall | ARID
Having or showing feelings of unwarranted importance | ARROGANT
An overwhelming feeling of wonder or admiration | AWE`,

        `Completely wanting or lacking | BARREN
A fire that can be seen from a distance | BEACON
Promoting or enhancing well|being | BENEFICIAL
An embarrassing mistake | BLUNDER
Marked by exuberance and high spirits | BOISTEROUS
Refusal to have commercial dealings with some organization | BOYCOTT
Weight to be carried or borne | BURDEN
Related operations aimed at achieving a particular goal | CAMPAIGN
A large alphabetic character used in writing or printing | CAPITAL
Relating to or arranged according to the order of time | CHRONOLOGICAL`,

        `Of or relating or belonging to a city | CIVIC
The quality of being coherent and easily understood | CLARITY
Work together on a common enterprise or project | COLLABORATE
Crash together with violent impact | COLLIDE
Present as worthy of regard, kindness, or confidence | COMMEND
A written explanation or criticism or illustration | COMMENTARY
Closely and firmly united or packed together | COMPACT
Steadiness of mind under stress | COMPOSURE
Expressing much in few words | CONCISE
Give an affirmative reply to; respond favorably to | CONSENT
A phenomenon that is caused by some previous phenomenon | CONSEQUENCE`,

        `Keep in safety and protect from harm, loss, or destruction | CONSERVE
Obvious to the eye or mind | CONSPICUOUS
Uninterrupted in time and indefinitely long continuing | CONSTANT
Make impure | CONTAMINATE
The set of facts or circumstances that surround a situation | CONTEXT
Moving in time or space without interruption | CONTINUOUS
A dispute where there is strong disagreement | CONTROVERSY
Suited to your comfort or purpose or needs | CONVENIENT
Come to terms with | COPE
Politely warm and friendly | CORDIAL`,

        `Adapt something wild to the environment | CULTIVATE
Increasing by successive addition | CUMULATIVE
State emphatically and authoritatively | DECLARE
A heavy rain | DELUGE`
      ]
    },
    {
      id: 'word|meanings-2',
      type: 'slides2',
      label: 'Word Meanings',
      commonData: {
        title: 'Word Meanings',
        displayType: 'custom',
        autoPlay: false,
        substep: false
      },
      data: [
        {
          steps: [
            [
              {
                type: 'title',
                text: 'idiom'
              },
              '- expression whose meaning cannot be inferred from its words',
              {
                type: 'html',
                text: "Eg: <br><br> 'Kick the bucket' is an <i><b>idiom</b></i> that means 'to die.'<br><br> Understanding <i><b>idioms</b></i> in a foreign language can be challenging."
              }
            ],
            [
              {
                type: 'title',
                text: 'ignite'
              },
              '- cause to start burning',
              {
                type: 'html',
                text: 'Eg: <br><br> He used a match to <i><b>ignite</b></i> the campfire.<br><br> A spark can <i><b>ignite</b></i> a fire in dry conditions.'
              }
            ],
            [
              {
                type: 'title',
                text: 'immense'
              },
              '- unusually great in size or amount or extent or scope',
              {
                type: 'html',
                text: "Eg: <br><br> The Grand Canyon's beauty is accentuated by its <i><b>immense</b></i> size.<br><br> The task ahead of us requires an <i><b>immense</b></i> amount of effort."
              }
            ],
            [
              {
                type: 'title',
                text: 'improvise'
              },
              '- manage in a makeshift way; do with whatever is at hand',
              {
                type: 'html',
                text: 'Eg: <br><br> When the chef ran out of ingredients, he had to <i><b>improvise</b></i> to complete the dish.<br><br> Sometimes, you need to <i><b>improvise</b></i> to solve unexpected problems.'
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'inept'
              },
              '- generally incompetent and ineffectual',
              {
                type: 'html',
                text: "Eg: <br><br> The <i><b>inept</b></i> handling of the crisis led to further problems.<br><br> An <i><b>inept</b></i> manager can harm a company's performance."
              }
            ],
            [
              {
                type: 'title',
                text: 'inevitable'
              },
              '- incapable of being avoided or prevented',
              {
                type: 'html',
                text: 'Eg: <br><br> Change is <i><b>inevitable</b></i>, and we must adapt to it.<br><br> The outcome was <i><b>inevitable</b></i> once the decision was made.'
              }
            ],
            [
              {
                type: 'title',
                text: 'influence'
              },
              '- a power to affect persons or events',
              {
                type: 'html',
                text: "Eg: <br><br> Her leadership had a strong <i><b>influence</b></i> on the team's success.<br><br> The media can <i><b>influence</b></i> public opinion."
              }
            ],
            [
              {
                type: 'title',
                text: 'ingenious'
              },
              '- showing inventiveness and skill',
              {
                type: 'html',
                text: 'Eg: <br><br> The <i><b>ingenious</b></i> design of the new smartphone impressed users.<br><br> Solving complex problems requires an <i><b>ingenious</b></i> approach.'
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'innovation'
              },
              '- the act of starting something for the first time',
              {
                type: 'html',
                text: "Eg: <br><br> The company's <i><b>innovation</b></i> in technology revolutionized the industry.<br><br> Entrepreneurship often involves <i><b>innovation</b></i> and risk-taking."
              }
            ],
            [
              {
                type: 'title',
                text: 'intimidate'
              },
              '- compel or deter by or as if by threats',
              {
                type: 'html',
                text: 'Eg: <br><br> The aggressive behavior of the bullies was meant to <i><b>intimidate</b></i> the students.<br><br> A peaceful demonstration should not be used to <i><b>intimidate</b></i> others.'
              }
            ],
            [
              {
                type: 'title',
                text: 'jovial'
              },
              '- full of or showing high-spirited merriment',
              {
                type: 'html',
                text: 'Eg: <br><br> The atmosphere at the party was <i><b>jovial</b></i>, with laughter and dancing.<br><br> His <i><b>jovial</b></i> personality brightened up the workplace.'
              }
            ],
            [
              {
                type: 'title',
                text: 'knack'
              },
              '- a special way of doing something',
              {
                type: 'html',
                text: 'Eg: <br><br> She has a <i><b>knack</b></i> for solving puzzles quickly.<br><br> His <i><b>knack</b></i> for public speaking makes him an effective communicator.'
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'leeway'
              },
              '- a permissible difference',
              {
                type: 'html',
                text: "Eg: <br><br> The chef allows some <i><b>leeway</b></i> in ingredient choices for the recipe.<br><br> There is a certain <i><b>leeway</b></i> in the project's deadline."
              }
            ],
            [
              {
                type: 'title',
                text: 'legislation'
              },
              '- the act of making or enacting laws',
              {
                type: 'html',
                text: 'Eg: <br><br> The <i><b>legislation</b></i> passed by the government aims to protect the environment.<br><br> The process of <i><b>legislation</b></i> requires careful consideration of the proposed laws.'
              }
            ],
            [
              {
                type: 'title',
                text: 'leisure'
              },
              '- time available for ease and relaxation',
              {
                type: 'html',
                text: 'Eg: <br><br> The weekend is a time of <i><b>leisure</b></i> when you can unwind and enjoy hobbies.<br><br> During his <i><b>leisure</b></i> hours, he likes to read and garden.'
              }
            ],
            [
              {
                type: 'title',
                text: 'liberate'
              },
              '- grant freedom to; free from confinement',
              {
                type: 'html',
                text: "Eg: <br><br> The soldiers helped <i><b>liberate</b></i> the oppressed people from the tyrant's rule.<br><br> The key <i><b>liberated</b></i> the bird from the cage."
              }
            ],
            [
              {
                type: 'title',
                text: 'likeness'
              },
              '- similarity in appearance or nature between persons or things',
              {
                type: 'html',
                text: 'Eg: <br><br> The <i><b>likeness</b></i> between the twins is striking; they look almost identical.<br><br> There is a <i><b>likeness</b></i> in character between the two main protagonists of the story.'
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'linger'
              },
              '- remain present although waning or gradually dying',
              {
                type: 'html',
                text: 'Eg: <br><br> The aroma of fresh-baked bread continued to <i><b>linger</b></i> in the kitchen.<br><br> Memories of that day still <i><b>linger</b></i> in my mind.'
              }
            ],
            [
              {
                type: 'title',
                text: 'literal'
              },
              '- limited to the explicit meaning of a word or text',
              {
                type: 'html',
                text: "Eg: <br><br> The phrase 'take a seat' is a <i><b>literal</b></i> instruction to sit down.<br><br> His interpretation of the poem was too <i><b>literal</b></i> and missed its deeper meaning."
              }
            ],
            [
              {
                type: 'title',
                text: 'loathe'
              },
              '- dislike intensely; feel disgust toward',
              {
                type: 'html',
                text: "Eg: <br><br> She <i><b>loathes</b></i> the taste of olives and won't eat anything with them.<br><br> He had a deep <i><b>loathe</b></i> for dishonesty."
              }
            ],
            [
              {
                type: 'title',
                text: 'lure'
              },
              '- provoke someone to do something through persuasion',
              {
                type: 'html',
                text: 'Eg: <br><br> The advertisement was designed to <i><b>lure</b></i> customers into trying the new product.<br><br> The treasure map <i><b>lured</b></i> adventurers to search for hidden riches.'
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'majority'
              },
              '- the main part',
              {
                type: 'html',
                text: "Eg: <br><br> The <i><b>majority</b></i> of the class agreed with the teacher's decision.<br><br> In a democratic system, the <i><b>majority</b></i> vote determines the outcome."
              }
            ],
            [
              {
                type: 'title',
                text: 'makeshift'
              },
              '- done or made using whatever is available',
              {
                type: 'html',
                text: 'Eg: <br><br> In the absence of a ladder, they created a <i><b>makeshift</b></i> one from wooden planks.<br><br> The <i><b>makeshift</b></i> campfire provided some warmth in the wilderness.'
              }
            ],
            [
              {
                type: 'title',
                text: 'manipulate'
              },
              '- influence or control shrewdly or deviously',
              {
                type: 'html',
                text: 'Eg: <br><br> The con artist tried to <i><b>manipulate</b></i> the victim into giving away personal information.<br><br> The political leaders attempted to <i><b>manipulate</b></i> public opinion.'
              }
            ],
            [
              {
                type: 'title',
                text: 'marvel'
              },
              '- be amazed at',
              {
                type: 'html',
                text: 'Eg: <br><br> The stunning architecture of the cathedral made visitors <i><b>marvel</b></i> at its beauty.<br><br> People <i><b>marvel</b></i> at the incredible abilities of athletes.'
              }
            ],
            [
              {
                type: 'title',
                text: 'massive'
              },
              '- containing a great quantity of matter',
              {
                type: 'html',
                text: 'Eg: <br><br> The mountain range is home to <i><b>massive</b></i> glaciers and peaks.<br><br> The <i><b>massive</b></i> stone walls of the castle are centuries old.'
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'maximum'
              },
              '- the greatest or most complete or best possible',
              {
                type: 'html',
                text: 'Eg: <br><br> The speed limit on this highway is <i><b>maximum</b></i> 70 miles per hour.<br><br> She performed at her <i><b>maximum</b></i> potential during the competition.'
              }
            ],
            [
              {
                type: 'title',
                text: 'meager'
              },
              '- deficient in amount or quality or extent',
              {
                type: 'html',
                text: "Eg: <br><br> His <i><b>meager</b></i> savings wouldn't be enough to cover the medical expenses.<br><br> The meal they offered was <i><b>meager</b></i> and unsatisfying."
              }
            ],
            [
              {
                type: 'title',
                text: 'mere'
              },
              '- being nothing more than specified',
              {
                type: 'html',
                text: "Eg: <br><br> He's a <i><b>mere</b></i> child; he doesn't have the strength to lift that heavy object.<br><br> The problem was of <i><b>mere</b></i> importance compared to the other issues."
              }
            ],
            [
              {
                type: 'title',
                text: 'migration'
              },
              '- the movement of persons from one locality to another',
              {
                type: 'html',
                text: 'Eg: <br><br> The <i><b>migration</b></i> of birds in the spring is a fascinating natural phenomenon.<br><br> Economic factors can drive the <i><b>migration</b></i> of people from rural areas to cities.'
              }
            ],
            [
              {
                type: 'title',
                text: 'mimic'
              },
              '- imitate, especially for satirical effect',
              {
                type: 'html',
                text: "Eg: <br><br> The comedian could <i><b>mimic</b></i> famous personalities with incredible accuracy.<br><br> He decided to <i><b>mimic</b></i> the president's speech for comedic effect."
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'minute'
              },
              '- a unit of time equal to 60 seconds or 1/60th of an hour',
              {
                type: 'html',
                text: 'Eg: <br><br> There are 60 <i><b>minutes</b></i> in an hour.<br><br> He arrived at the meeting with only a <i><b>minute</b></i> to spare.'
              }
            ],
            [
              {
                type: 'title',
                text: 'monotonous'
              },
              '- sounded or spoken in a tone unvarying in pitch',
              {
                type: 'html',
                text: 'Eg: <br><br> The <i><b>monotonous</b></i> sound of the dripping faucet was irritating.<br><br> Her <i><b>monotonous</b></i> speech put the audience to sleep.'
              }
            ],
            [
              {
                type: 'title',
                text: 'negotiate'
              },
              '- discuss the terms of an arrangement',
              {
                type: 'html',
                text: 'Eg: <br><br> They had to <i><b>negotiate</b></i> the terms of the contract before signing it.<br><br> Diplomats from both countries are meeting to <i><b>negotiate</b></i> a peace agreement.'
              }
            ],
            [
              {
                type: 'title',
                text: 'obstacle'
              },
              '- something that stands in the way and must be surmounted',
              {
                type: 'html',
                text: 'Eg: <br><br> The lack of funds was a significant <i><b>obstacle</b></i> to completing the project.<br><br> He viewed every challenge as an <i><b>obstacle</b></i> to overcome.'
              }
            ],
            [
              {
                type: 'title',
                text: 'omniscient'
              },
              '- knowing, seeing, or understanding everything',
              {
                type: 'html',
                text: 'Eg: <br><br> Some believe that a deity is <i><b>omniscient</b></i> and has complete knowledge of the universe.<br><br> In the story, the wise old character was portrayed as <i><b>omniscient</b></i>.'
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'onset'
              },
              '- the beginning or early stages',
              {
                type: 'html',
                text: 'Eg: <br><br> The <i><b>onset</b></i> of winter brings cold temperatures and snow.<br><br> In the <i><b>onset</b></i> of the disease, early detection is crucial.'
              }
            ],
            [
              {
                type: 'title',
                text: 'optimist'
              },
              '- a person disposed to take a favorable view of things',
              {
                type: 'html',
                text: 'Eg: <br><br> Despite the challenges, she remained an <i><b>optimist</b></i> and saw the potential for positive change.<br><br> The <i><b>optimist</b></i> believed that things would work out for the best.'
              }
            ],
            [
              {
                type: 'title',
                text: 'originate'
              },
              '- come into existence; take on form or shape',
              {
                type: 'html',
                text: 'Eg: <br><br> The traditional dance <i><b>originated</b></i> in this region centuries ago.<br><br> Ideas can <i><b>originate</b></i> from creative thinking and inspiration.'
              }
            ],
            [
              {
                type: 'title',
                text: 'painstaking'
              },
              '- characterized by extreme care and great effort',
              {
                type: 'html',
                text: "Eg: <br><br> The artist's <i><b>painstaking</b></i> attention to detail is evident in every brushstroke.<br><br> Writing a research paper requires <i><b>painstaking</b></i> research and analysis."
              }
            ],
            [
              {
                type: 'title',
                text: 'paraphrase'
              },
              '- express the same message in different words',
              {
                type: 'html',
                text: 'Eg: <br><br> Can you <i><b>paraphrase</b></i> this paragraph to make it simpler to understand?<br><br> The teacher asked the students to <i><b>paraphrase</b></i> the poem.'
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'parody'
              },
              '- a composition that imitates or misrepresents a style',
              {
                type: 'html',
                text: 'Eg: <br><br> The comedy show featured a hilarious <i><b>parody</b></i> of a popular movie.<br><br> The book is a clever <i><b>parody</b></i> of classic fairy tales.'
              }
            ],
            [
              {
                type: 'title',
                text: 'persecute'
              },
              '- cause to suffer',
              {
                type: 'html',
                text: 'Eg: <br><br> The dictator used his power to <i><b>persecute</b></i> political dissidents.<br><br> Discrimination can <i><b>persecute</b></i> individuals based on their race or religion.'
              }
            ],
            [
              {
                type: 'title',
                text: 'plummet'
              },
              '- drop sharply',
              {
                type: 'html',
                text: "Eg: <br><br> The stock prices began to <i><b>plummet</b></i> after the company's financial troubles were revealed.<br><br> The temperature can <i><b>plummet</b></i> during the winter months."
              }
            ],
            [
              {
                type: 'title',
                text: 'possess'
              },
              '- have ownership of',
              {
                type: 'html',
                text: "Eg: <br><br> She <i><b>possesses</b></i> a vast collection of antique coins.<br><br> To <i><b>possess</b></i> a driver's license, you must pass the required tests."
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'poverty'
              },
              '- the state of having little or no money and possessions',
              {
                type: 'html',
                text: 'Eg: <br><br> Many charitable organizations work to alleviate <i><b>poverty</b></i> in underprivileged communities.<br><br> Extreme <i><b>poverty</b></i> can lead to a lack of basic necessities.'
              }
            ],
            [
              {
                type: 'title',
                text: 'precise'
              },
              '- sharply exact or accurate or delimited',
              {
                type: 'html',
                text: "Eg: <br><br> The scientist made <i><b>precise</b></i> measurements to ensure the experiment's accuracy.<br><br> Following the <i><b>precise</b></i> instructions is essential for safety."
              }
            ],
            [
              {
                type: 'title',
                text: 'predicament'
              },
              '- an unpleasant or difficult situation',
              {
                type: 'html',
                text: 'Eg: <br><br> He found himself in a <i><b>predicament</b></i> when his car broke down in the middle of nowhere.<br><br> Facing a financial <i><b>predicament</b></i> can be stressful.'
              }
            ],
            [
              {
                type: 'title',
                text: 'predict'
              },
              '- make a guess about what will happen in the future',
              {
                type: 'html',
                text: "Eg: <br><br> Meteorologists can <i><b>predict</b></i> the weather based on data and patterns.<br><br> It's challenging to <i><b>predict</b></i> the outcome of a complex situation."
              }
            ],
            [
              {
                type: 'title',
                text: 'prejudice'
              },
              '- a partiality preventing objective consideration of an issue',
              {
                type: 'html',
                text: 'Eg: <br><br> Prejudice can lead to unfair treatment of individuals based on their background.<br><br> Overcoming <i><b>prejudice</b></i> is essential for a just society.'
              }
            ]
          ]
        }
      ]
    },
    {
      label: 'Fill in the blanks',
      type: 'matchByDragDrop',
      id: 'drag-drop-2',
      lockAfter: 1,
      commonData: {
        title:
          'Drag the correct option in each blank to form meaningfull sentences.'
      },
      data: [
        `Can you pass me the lighter so I can *ignite* the campfire?
The Grand Canyon's beauty is accentuated by its *immense* size.
When the chef ran out of ingredients, he had to *improvise* to complete the dish.
The *inept* handling of the crisis worsened the situation.
*Inevitable* changes require us to adapt and evolve.`,

        `Her leadership exerted a strong *influence* on the team's success.
The smartphone's *ingenious* design left users impressed.
The company's *innovation* revolutionized the technology industry.
The aggressive behavior of the bullies was meant to *intimidate* the students.
The atmosphere at the party was *jovial*, filled with laughter and dancing.`,

        `She has a *knack* for quickly solving puzzles.
The chef allows some *leeway* in ingredient choices for the recipe.
The government's *legislation* aims to protect the environment.
The weekend is a time of *leisure*, a moment to unwind and enjoy hobbies.
The soldiers helped *liberate* the oppressed people from the tyrant's rule.`,

        `The *likeness* between the twins is striking; they look almost identical.
The aroma of fresh-baked bread continued to *linger* in the kitchen.
The phrase 'take a seat' is a *literal* instruction to sit down.
She absolutely *loathes* the taste of olives and refuses to eat anything with them.
The advertisement was designed to *lure* customers into trying the new product.`,

        `*Majority* of the class agreed with the teacher's decision.
In the absence of a ladder, they created a *makeshift* one from wooden planks.
The con artist tried to *manipulate* the victim into giving away personal information.
The stunning architecture of the cathedral made visitors *marvel* at its beauty.
The mountain range is home to *massive* glaciers and peaks.`,

        `The *maximum* speed limit on this highway is 70 miles per hour.
His *meager* savings wouldn't be enough to cover the medical expenses.
He's a *mere* child; he doesn't have the strength to lift that heavy object.
The *migration* of birds in the spring is a fascinating natural phenomenon.
The comedian could *mimic* famous personalities with incredible accuracy.`,

        `There are 60 *minutes* in an hour.
The *monotonous* sound of the dripping faucet was irritating.
They had to *negotiate* the terms of the contract before signing it.
The lack of funds was a significant *obstacle* to completing the project.
Some believe that a deity is *omniscient* and has complete knowledge of the universe.`,

        `The *onset* of winter brings cold temperatures and snow.
Her *outlandish* fashion sense always drew attention.
The news of their victory would *overwhelm* them with joy.
The artist's *painstaking* attention to detail is visible in every brushstroke.
The comedy show included a hilarious *parody* of a popular movie.`,

        `The dictator abused his power to *persecute* political dissidents.
The stock prices started to *plummet* after the company's financial troubles were revealed.
She *possesses* a vast collection of antique coins.
Many charitable organizations work to alleviate *poverty* in underprivileged communities.
The scientist made *precise* measurements to ensure the experiment's accuracy.`,

        `He found himself in a *predicament* when his car broke down in the middle of nowhere.
Meteorologists can *predict* the weather based on data and patterns.
*Prejudice* can lead to unfair treatment of individuals based on their background.
The team had an initial meeting to discuss the project's key points.
The cave paintings are an example of *primitive* art from ancient times.`
      ]
    },
    {
      type: 'completeWord',
      id: 'complete-word-2',
      label: 'Complete Word',
      data: [
        `An expression whose meaning cannot be inferred from its words | IDIOM
Cause to start burning | IGNITE
Unusually great in size or amount or extent or scope | IMMENSE
Manage in a makeshift way; do with whatever is at hand | IMPROVISE
Generally incompetent and ineffectual | INEPT
Incapable of being avoided or prevented | INEVITABLE
A power to affect persons or events | INFLUENCE
Showing inventiveness and skill | INGENIOUS
The act of starting something for the first time | INNOVATION
Compel or deter by or as if by threats | INTIMIDATE`,

        `Full of or showing high-spirited merriment | JOVIAL
A special way of doing something | KNACK
A permissible difference | LEEWAY
The act of making or enacting laws | LEGISLATION
Time available for ease and relaxation | LEISURE
Grant freedom to; free from confinement | LIBERATE
Similarity in appearance or nature between persons or things | LIKENESS
Remain present although waning or gradually dying | LINGER
Limited to the explicit meaning of a word or text | LITERAL
Dislike intensely; feel disgust toward | LOATHE`,

        `Provoke someone to do something through persuasion | LURE
The main part | MAJORITY
Done or made using whatever is available | MAKESHIFT
Influence or control shrewdly or deviously | MANIPULATE
Be amazed at | MARVEL
Containing a great quantity of matter | MASSIVE
The greatest or most complete or best possible | MAXIMUM
Deficient in amount or quality or extent | MEAGER
Being nothing more than specified | MERE
The movement of persons from one locality to another | MIGRATION`,

        `Imitate, especially for satirical effect | MIMIC
A unit of time equal to 60 seconds or 1/60th of an hour | MINUTE
Sounded or spoken in a tone unvarying in pitch | MONOTONOUS
Discuss the terms of an arrangement | NEGOTIATE
Something that stands in the way and must be surmounted | OBSTACLE
Knowing, seeing, or understanding everything | OMNISCIENT
The beginning or early stages | ONSET
A person disposed to take a favorable view of things | OPTIMIST
Come into existence; take on form or shape | ORIGINATE
Characterized by extreme care and great effort | PAINSTAKING`,

        `Express the same message in different words | PARAPHRASE
A composition that imitates or misrepresents a style | PARODY
Cause to suffer | PERSECUTE
Drop sharply | PLUMMET
Have ownership of | POSSESS
The state of having little or no money and possessions | POVERTY
Sharply exact or accurate or delimited | PRECISE
An unpleasant or difficult situation | PREDICAMENT
Make a guess about what will happen in the future | PREDICT
A partiality preventing objective consideration of an issue | PREJUDICE`
      ]
    }
  ]
};
