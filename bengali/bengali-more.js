export default {
  id: 'bengali-more',
  label: 'More Bengali',
  type: 'curriculumList',
  list: [
    {
      id: 'more',
      list: [
        {
          label: 'শব্দ 2',
          id: 'bn-word-3'
        },
        {
          label: 'শব্দ 3',
          id: 'bn-word-5'
        },
        {
          label: 'ব্যাকরণ',
          id: 'bn-grammar-4'
        },
        {
          label: 'ছোটদের ছড়া',
          id: 'bn-poem'
        },
        {
          label: 'কবিতা সহজপাঠ',
          id: 'bn-poem-1'
        },
        {
          label: 'গল্প',
          id: 'bn-reading-2'
        },
        {
          label: 'সুকুমার রায় - গল্প ',
          id: 'bn-sukumar-story'
        },
        {
          label: 'বর্ণ অনুশীলন',
          id: 'bn-letter-2'
        },
        {
          label: 'ছবি: ফল এবং শাকসবজি',
          id: 'bn-picture-2'
        },
        {
          label: 'ছবি: প্রকৃতি এবং অন্যান্য জিনিস',
          id: 'bn-picture-3'
        },
        {
          label: 'ছবি: ঘর এবং অন্যান্য বিষয়',
          id: 'bn-picture-4'
        },
        {
          label: 'ছবি: কাজ ও খেলা',
          id: 'bn-picture-5'
        },
        {
          label: 'ছবি: আরো পশু এবং পাখি',
          id: 'bn-picture-6'
        },
        {
          label: 'ছবি: আরো ফল এবং সবজি',
          id: 'bn-picture-7'
        },
        {
          label: 'ছবি: যানবাহন এবং চাকরি',
          id: 'bn-picture-8'
        },
        {
          label: 'ছবি: আরো ছবি',
          id: 'bn-picture-9'
        },
        {
          label: 'ছবি: সামাজিক ও বিজ্ঞান',
          id: 'bn-picture-10'
        },
        {
          label: 'ছবি: সরঞ্জাম এবং বাদ্যযন্ত্র',
          id: 'bn-picture-11'
        },
        {
          label: 'ধ্বনি: ফল এবং শাকসবজি',
          id: 'bn-sound-2'
        },
        {
          label: 'ধ্বনি: প্রকৃতি এবং অন্যান্য জিনিস',
          id: 'bn-sound-3'
        },
        {
          label: 'ধ্বনি: ঘর এবং অন্যান্য বিষয়',
          id: 'bn-sound-4'
        },
        {
          label: 'ধ্বনি: কাজ ও খেলা',
          id: 'bn-sound-5'
        },
        {
          label: 'ধ্বনি: আরো পশু এবং পাখি',
          id: 'bn-sound-6'
        },
        {
          label: 'ধ্বনি: আরো ফল এবং সবজি',
          id: 'bn-sound-7'
        },
        {
          label: 'ধ্বনি: যানবাহন এবং চাকরি',
          id: 'bn-sound-8'
        },
        {
          label: 'ধ্বনি: আরো ছবি',
          id: 'bn-sound-9'
        },
        {
          label: 'ধ্বনি: সামাজিক ও বিজ্ঞান',
          id: 'bn-sound-10'
        },
        {
          label: 'ধ্বনি: সরঞ্জাম এবং বাদ্যযন্ত্র',
          id: 'bn-sound-11'
        },

        {
          label: 'Find Word from Picture: Animals and Birds',
          id: 'bn-pic-word'
        },
        {
          label: 'Find Word from Picture: Fruits and Vegetables',
          id: 'bn-pic-word-2'
        },
        {
          label: 'Find Word from Picture: Nature and Others',
          id: 'bn-pic-word-3'
        },
        {
          label: 'Find Word from Picture: House and Others',
          id: 'bn-pic-word-4'
        },
        {
          label: 'Find Word from Picture: Action and Sports',
          id: 'bn-pic-word-5'
        },
        {
          label: 'Find Word from Picture: More Animals and Birds',
          id: 'bn-pic-word-6'
        },
        {
          label: 'Find Word from Picture: More Fruits and Vegetables',
          id: 'bn-pic-word-7'
        },
        {
          label: 'Find Word from Picture: Vehicles and Jobs',
          id: 'bn-pic-word-8'
        },
        {
          label: 'Find Word from Picture: More Images',
          id: 'bn-pic-word-9'
        },
        {
          label: 'Find Word from Picture: Social and Science',
          id: 'bn-pic-word-10'
        },
        {
          label: 'Find Word from Picture: Tools and Musical Instruments',
          id: 'bn-pic-word-11'
        },
        {
          label: 'English Words: Fruits and Vegetables',
          id: 'bn-en-2'
        },
        {
          label: 'English Words: Nature and Others',
          id: 'bn-en-3'
        },
        {
          label: 'English Words: House and Others',
          id: 'bn-en-4'
        },
        {
          label: 'English Words: Action and Sports',
          id: 'bn-en-5'
        },
        {
          label: 'English Words: More Animals and Birds',
          id: 'bn-en-6'
        },
        {
          label: 'English Words: More fruits and vegetables',
          id: 'bn-en-7'
        },
        {
          label: 'English Words: Vehicles and Jobs',
          id: 'bn-en-8'
        },
        {
          label: 'English Words: More Images',
          id: 'bn-en-9'
        },
        {
          label: 'English Words: Social and Science',
          id: 'bn-en-10'
        },
        {
          label: 'English Words: Tools and Musical Instruments',
          id: 'bn-en-11'
        }
      ]
    }
  ]
};
