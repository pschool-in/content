export default {
  label: 'Word Meanings',
  id: 'word-meaning-5',
  lockAfter: 2,
  list: [
    {
      id: 'meanings-1',
      type: 'slides2',
      label: `Word Meanings`,
      commonData: {
        title: `Word Meanings`,
        displayType: 'custom',
        autoPlay: false,
        substep: false
      },
      data: [
        {
          steps: [
            [
              {
                type: 'title',
                text: 'accurate'
              },
              '- characterized by perfect conformity to fact or truth',
              {
                type: 'html',
                text: `Eg: <br><br>
The scientist's measurements were highly <i><b>accurate</b></i>, with minimal margin for error.<br><br>
A good dictionary provides <i><b>accurate</b></i> definitions and examples of words.`
              }
            ],
            [
              {
                type: 'title',
                text: 'address'
              },
              '- to speak or write to someone',
              {
                type: 'html',
                text: `Eg: <br><br>
    I will <i><b>address</b></i> the issue with the customer and find a solution.<br><br>
    She <i><b>addressed</b></i> the audience with a heartfelt speech.`
              }
            ],
            [
              {
                type: 'title',
                text: 'afford'
              },
              '- have the financial means to do something or buy something',
              {
                type: 'html',
                text: `Eg: <br><br>
After years of hard work, they could finally <i><b>afford</b></i> their dream vacation.<br><br>
It's important to <i><b>afford</b></i> opportunities for education to all, regardless of their background.`
              }
            ],
            [
              {
                type: 'title',
                text: 'alert'
              },
              '- warn or arouse to a sense of danger',
              {
                type: 'html',
                text: `Eg: <br><br>
The lifeguard's job is to <i><b>alert</b></i> swimmers to potential dangers in the water.<br><br>
An <i><b>alert</b></i> was issued to residents about an approaching storm.`
              }
            ],
            [
              {
                type: 'title',
                text: 'analyze'
              },
              '- carefully observe, investigate, or study something to gain a deeper understanding',
              {
                type: 'html',
                text: `Eg: <br><br>
Scientists <i><b>analyze</b></i> data to draw meaningful conclusions from their research.<br><br>
The detective needed to <i><b>analyze</b></i> the evidence to solve the mystery.`
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'ancestor'
              },
              '- individuals to whom a person is related by blood or descent',
              {
                type: 'html',
                text: `Eg: <br><br>
Genealogy enthusiasts often research their <i><b>ancestors</b></i> to learn about their family history.<br><br>
Our distant <i><b>ancestors</b></i> faced many challenges in their daily lives.`
              }
            ],
            [
              {
                type: 'title',
                text: 'annual'
              },
              '- occurring every year',
              {
                type: 'html',
                text: `Eg: <br><br>
The <i><b>annual</b></i> holiday parade is a beloved tradition in our town.<br><br>
The company holds its <i><b>annual</b></i> shareholders' meeting to discuss business matters.`
              }
            ],
            [
              {
                type: 'title',
                text: 'apparent'
              },
              '- clear, obvious, easily seen or understood',
              {
                type: 'html',
                text: `Eg: <br><br>
The smile on her face made her happiness <i><b>apparent</b></i> to everyone in the room.<br><br>
The reasons for the problem became <i><b>apparent</b></i> after a thorough investigation.`
              }
            ],
            [
              {
                type: 'title',
                text: 'appropriate'
              },
              '- suitable for a particular person, place, or situation',
              {
                type: 'html',
                text: `Eg: <br><br>
It's important to choose <i><b>appropriate</b></i> clothing for different weather conditions.<br><br>
Using <i><b>appropriate</b></i> language is essential in professional communication.`
              }
            ],
            [
              {
                type: 'title',
                text: 'arena'
              },
              '- a large stage for open-air sports or entertainments',
              {
                type: 'html',
                text: `Eg: <br><br>
The <i><b>arena</b></i> was filled with excited fans cheering for their favorite sports team.<br><br>
The new <i><b>arena</b></i> hosts concerts, sports events, and cultural performances.`
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'arrest'
              },
              '- take into custody',
              {
                type: 'html',
                text: `Eg: <br><br>
The police had to <i><b>arrest</b></i> the suspect after a thorough investigation.<br><br>
The <i><b>arrest</b></i> was made in accordance with the law and legal procedures.`
              }
            ],
            [
              {
                type: 'title',
                text: 'ascend'
              },
              '- travel up',
              {
                type: 'html',
                text: `Eg: <br><br>
Climbers often <i><b>ascend</b></i> steep mountains with great determination and skill.<br><br>
The hot air balloon began to <i><b>ascend</b></i> into the clear blue sky.`
              }
            ],
            [
              {
                type: 'title',
                text: 'assist'
              },
              '- give help; be of service',
              {
                type: 'html',
                text: `Eg: <br><br>
Volunteers <i><b>assist</b></i> in disaster relief efforts to provide support to affected communities.<br><br>
A friendly neighbor was always willing to <i><b>assist</b></i> with household tasks.`
              }
            ],
            [
              {
                type: 'title',
                text: 'attempt'
              },
              '- make an effort',
              {
                type: 'html',
                text: `Eg: <br><br>
Despite the challenges, she was determined to <i><b>attempt</b></i> her dream of starting a business.<br><br>
An <i><b>attempt</b></i> to set a new world record was met with excitement and anticipation.`
              }
            ],
            [
              {
                type: 'title',
                text: 'attractive'
              },
              '- pleasing to the eye or mind',
              {
                type: 'html',
                text: `Eg: <br><br>
The picturesque landscape of the countryside was incredibly <i><b>attractive</b></i>.<br><br>
A well-designed website is more <i><b>attractive</b></i> to users and visitors.`
              }
            ]
          ]
        },

        {
          steps: [
            [
              {
                type: 'title',
                text: 'balance'
              },
              '- a state of equilibrium or equipoise; equal distribution of weight',
              {
                type: 'html',
                text: `Eg: <br><br>
The gymnast demonstrated incredible <i><b>balance</b></i> on the narrow beam.<br><br>
Maintaining a <i><b>balance</b></i> between work and personal life is essential for well-being.`
              }
            ],
            [
              {
                type: 'title',
                text: 'barrier'
              },
              '- a structure or object that blocks free movement',
              {
                type: 'html',
                text: `Eg: <br><br>
The wall served as a <i><b>barrier</b></i> to keep intruders out of the property.<br><br>
Language should not be a <i><b>barrier</b></i> to understanding and connecting with others.`
              }
            ],
            [
              {
                type: 'title',
                text: 'benefit'
              },
              '- something that promotes well-being',
              {
                type: 'html',
                text: `Eg: <br><br>
Regular exercise has many <i><b>benefits</b></i> for physical and mental health.<br><br>
One of the <i><b>benefits</b></i> of the new policy is improved access to healthcare.`
              }
            ],
            [
              {
                type: 'title',
                text: 'bias'
              },
              '- a partiality that prevents objective consideration of an issue or situation',
              {
                type: 'html',
                text: `Eg: <br><br>
It's important for journalists to avoid <i><b>bias</b></i> and report the news objectively.<br><br>
The judge's decision was made without any <i><b>bias</b></i> or prejudice.`
              }
            ],
            [
              {
                type: 'title',
                text: 'bond'
              },
              '- a connection that fastens things together',
              {
                type: 'html',
                text: `Eg: <br><br>
The strong <i><b>bond</b></i> between the two friends was evident in their unwavering support for each other.<br><br>
Chemical reactions often involve the breaking and forming of <i><b>bonds</b></i> between atoms.`
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'brief'
              },
              '- of short duration or distance',
              {
                type: 'html',
                text: `Eg: <br><br>
The meeting was <i><b>brief</b></i>, lasting only 15 minutes.<br><br>
She provided a <i><b>brief</b></i> summary of the main points in the report.`
              }
            ],
            [
              {
                type: 'title',
                text: 'bulk'
              },
              '- indicates that something is large or takes up a significant part of something',
              {
                type: 'html',
                text: `Eg: <br><br>
The <i><b>bulk</b></i> of the project involved extensive research and data analysis.<br><br>
Buying items in <i><b>bulk</b></i> can often save money in the long run.`
              }
            ],
            [
              {
                type: 'title',
                text: 'capacity'
              },
              '- the ability to perform or produce',
              {
                type: 'html',
                text: `Eg: <br><br>
The new factory has the <i><b>capacity</b></i> to produce thousands of units per day.<br><br>
A person's <i><b>capacity</b></i> to learn and adapt is remarkable.`
              }
            ],
            [
              {
                type: 'title',
                text: 'category'
              },
              '- a way to group or classify things that share certain characteristics or attributes',
              {
                type: 'html',
                text: `Eg: <br><br>
There are numerous <i><b>categories</b></i> of books in the library, including fiction, non-fiction, and reference.<br><br>
The food competition had a <i><b>category</b></i> for the best dessert creation.`
              }
            ],
            [
              {
                type: 'title',
                text: 'cease'
              },
              '- put an end to a state or an activity',
              {
                type: 'html',
                text: `Eg: <br><br>
The cease-fire agreement brought a temporary <i><b>cease</b></i> to the conflict.<br><br>
She decided to <i><b>cease</b></i> her involvement in the project due to other commitments.`
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'challenge'
              },
              '- a demanding or stimulating situation',
              {
                type: 'html',
                text: `Eg: <br><br>
Solving complex puzzles can be a great <i><b>challenge</b></i> for the mind.<br><br>
Overcoming this obstacle was the <i><b>challenge</b></i> of a lifetime.`
              }
            ],
            [
              {
                type: 'title',
                text: 'channel'
              },
              '- a pathway through which information flows',
              {
                type: 'html',
                text: `Eg: <br><br>
The internet is a powerful <i><b>channel</b></i> for sharing knowledge and connecting people worldwide.<br><br>
She changed the <i><b>channel</b></i> on the television to find a different program.`
              }
            ],
            [
              {
                type: 'title',
                text: 'characteristic'
              },
              '- a distinguishing quality or trait',
              {
                type: 'html',
                text: `Eg: <br><br>
Honesty is a key <i><b>characteristic</b></i> of a trustworthy person.<br><br>
One <i><b>characteristic</b></i> of tropical climates is high humidity.`
              }
            ],
            [
              {
                type: 'title',
                text: 'cite'
              },
              '- to quote as an authority or example',
              {
                type: 'html',
                text: `Eg: <br><br>
In your research paper, remember to <i><b>cite</b></i> your sources to give credit to the original authors.<br><br>
The author <i><b>cited</b></i> several historical documents to support the argument.`
              }
            ],
            [
              {
                type: 'title',
                text: 'civil'
              },
              '- relating to ordinary citizens and their concerns',
              {
                type: 'html',
                text: `Eg: <br><br>
    <i><b>Civil</b></i> rights movements have played a crucial role in achieving equality for all people.<br><br>
The dispute was resolved through <i><b>civil</b></i> negotiations rather than legal action.`
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'clarify'
              },
              '- to make clear and understandable',
              {
                type: 'html',
                text: `Eg: <br><br>
Can you <i><b>clarify</b></i> your instructions? I'm not sure what you mean.<br><br>
The manager needed to <i><b>clarify</b></i> the company's new policy for the employees.`
              }
            ],
            [
              {
                type: 'title',
                text: 'classic'
              },
              '- something that has stood the test of time and remains relevant, valuable, or appreciated across generations',
              {
                type: 'html',
                text: `Eg: <br><br>
"To Kill a Mockingbird" is considered a <i><b>classic</b></i> novel of American literature.<br><br>
Classic cars from the 1950s are highly sought after by collectors.`
              }
            ],
            [
              {
                type: 'title',
                text: 'coincide'
              },
              '- to occur at the same time or place',
              {
                type: 'html',
                text: `Eg: <br><br>
Our vacation plans <i><b>coincide</b></i> with the local festival, so we'll get to experience it.<br><br>
The meeting will <i><b>coincide</b></i> with her business trip, making it convenient for her.`
              }
            ],
            [
              {
                type: 'title',
                text: 'collapse'
              },
              '- to fall down or give way suddenly',
              {
                type: 'html',
                text: `Eg: <br><br>
The old building was in such disrepair that it was in danger of <i><b>collapse</b></i>.<br><br>
The stock market saw a sudden <i><b>collapse</b></i> due to economic factors.`
              }
            ],
            [
              {
                type: 'title',
                text: 'colleague'
              },
              '- an associate or coworker, especially in a professional setting',
              {
                type: 'html',
                text: `Eg: <br><br>
My <i><b>colleague</b></i> and I have been collaborating on this project for several months.<br><br>
She introduced me to her <i><b>colleagues</b></i> at the conference.`
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'calculate'
              },
              '- make a mathematical computation',
              {
                type: 'html',
                text: `Eg: <br><br>
Students need to <i><b>calculate</b></i> the total sum of the numbers in this equation.<br><br>
To determine the cost, you must <i><b>calculate</b></i> the expenses and revenues.`
              }
            ],
            [
              {
                type: 'title',
                text: 'capable'
              },
              '- having ability',
              {
                type: 'html',
                text: `Eg: <br><br>
She is <i><b>capable</b></i> of handling challenging tasks with ease.<br><br>
The team members are all highly <i><b>capable</b></i> professionals in their respective fields.`
              }
            ],
            [
              {
                type: 'title',
                text: 'captivity'
              },
              '- the state of being imprisoned',
              {
                type: 'html',
                text: `Eg: <br><br>
The animals were kept in <i><b>captivity</b></i> for research purposes.<br><br>
The explorer described his experiences in <i><b>captivity</b></i> during his expedition.`
              }
            ],
            [
              {
                type: 'title',
                text: 'carefree'
              },
              '- without trouble or worry',
              {
                type: 'html',
                text: `Eg: <br><br>
The children played <i><b>carefree</b></i> in the park, enjoying the sunny day.<br><br>
During the vacation, we embraced a <i><b>carefree</b></i> attitude and left our worries behind.`
              }
            ],
            [
              {
                type: 'title',
                text: 'century'
              },
              '- a period of 100 years',
              {
                type: 'html',
                text: `Eg: <br><br>
The historical artifact dates back to the 18th <i><b>century</b></i>.<br><br>
We've entered a new <i><b>century</b></i> filled with technological advancements.`
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'chamber'
              },
              '- a natural or artificial enclosed space',
              {
                type: 'html',
                text: `Eg: <br><br>
The hidden treasure was found in a secret <i><b>chamber</b></i> deep within the castle.<br><br>
The sound echoed through the empty <i><b>chamber</b></i>.`
              }
            ],
            [
              {
                type: 'title',
                text: 'circular'
              },
              '- having a circular shape',
              {
                type: 'html',
                text: `Eg: <br><br>
The Earth follows a <i><b>circular</b></i> orbit around the Sun.<br><br>
The wheels of the bicycle were <i><b>circular</b></i> in design.`
              }
            ],
            [
              {
                type: 'title',
                text: 'coax'
              },
              '- influence or persuade by gentle and persistent urging',
              {
                type: 'html',
                text: `Eg: <br><br>
She tried to <i><b>coax</b></i> her cat out of hiding with a treat.<br><br>
The manager had to <i><b>coax</b></i> the team into accepting the new project.`
              }
            ],
            [
              {
                type: 'title',
                text: 'column'
              },
              '- a line of units following one after another',
              {
                type: 'html',
                text: `Eg: <br><br>
The newspaper has a regular <i><b>column</b></i> for opinion pieces.<br><br>
The soldiers marched in a straight <i><b>column</b></i>.`
              }
            ],
            [
              {
                type: 'title',
                text: 'communicate'
              },
              '- convey or share information, thoughts, ideas, or feelings with someone else or a group of people',
              {
                type: 'html',
                text: `Eg: <br><br>
People use various forms of language to <i><b>communicate</b></i> with each other.<br><br>
The new technology makes it easier to <i><b>communicate</b></i> across long distances.`
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'competition'
              },
              '- the act of contending with others for rewards or resources',
              {
                type: 'html',
                text: `Eg: <br><br>
The Olympic Games are a showcase of international <i><b>competition</b></i>.<br><br>
Businesses often engage in <i><b>competition</b></i> to gain market share.`
              }
            ],
            [
              {
                type: 'title',
                text: 'complete'
              },
              '- having all necessary qualities',
              {
                type: 'html',
                text: `Eg: <br><br>
The puzzle is not <i><b>complete</b></i> without all the missing pieces.<br><br>
To succeed, you need a <i><b>complete</b></i> understanding of the subject.`
              }
            ],
            [
              {
                type: 'title',
                text: 'concentrate'
              },
              '- make denser, stronger, or purer; giving full and undivided attention to something',
              {
                type: 'html',
                text: `Eg: <br><br>
The chef had to <i><b>concentrate</b></i> the sauce to enhance its flavor.<br><br>
It's important to <i><b>concentrate</b></i> on your studies to excel in your exams.`
              }
            ],
            [
              {
                type: 'title',
                text: 'concern'
              },
              '- something that interests you because it is important',
              {
                type: 'html',
                text: `Eg: <br><br>
Environmental <i><b>concerns</b></i> have led to a push for sustainability.<br><br>
The safety of the employees is a top <i><b>concern</b></i> for the company.`
              }
            ],
            [
              {
                type: 'title',
                text: 'conclude'
              },
              '- bring to a close',
              {
                type: 'html',
                text: `Eg: <br><br>
The meeting will <i><b>conclude</b></i> with a summary of the decisions made.<br><br>
At the end of the trial, the judge will <i><b>conclude</b></i> the proceedings.`
              }
            ]
          ]
        }
      ]
    },
    {
      label: 'Fill in the blanks',
      type: 'matchByDragDrop',
      id: 'drag-drop-1',
      lockAfter: 1,
      commonData: {
        title:
          'Drag the correct option in each blank to form meaningfull sentences.'
      },
      data: [
        `The scientist's measurements were highly *accurate*, with minimal margin for error.
I will *address* the issue with the customer and find a solution.
After years of hard work, they could finally *afford* their dream vacation.
The lifeguard's job is to *alert* swimmers to potential dangers in the water.
Scientists *analyze* data to draw meaningful conclusions from their research.`,

        `Genealogy enthusiasts often research their *ancestors* to learn about their family history.
The *annual* holiday parade is a beloved tradition in our town.
The smile on her face made her happiness *apparent* to everyone in the room.
It's important to choose *appropriate* clothing for different weather conditions.
The *arena* was filled with excited fans cheering for their favorite sports team.`,

        `The police had to *arrest* the suspect after a thorough investigation.
Climbers often *ascend* steep mountains with great determination and skill.
Volunteers *assist* in disaster relief efforts to provide support to affected communities.
Despite the challenges, she was determined to *attempt* her dream of starting a business.
The picturesque landscape of the countryside was incredibly *attractive*.`,

        `The gymnast demonstrated incredible *balance* on the narrow beam.
The wall served as a *barrier* to keep intruders out of the property.
Regular exercise has many *benefits* for physical and mental health.
It's important for journalists to avoid *bias* and report the news objectively.
The strong *bond* between the two friends was evident in their unwavering support for each other.`,

        `The meeting was *brief*, lasting only 15 minutes.
The *bulk* of the project involved extensive research and data analysis.
The new factory has the *capacity* to produce thousands of units per day.
There are numerous *categories* of books in the library, including fiction, non-fiction, and reference.
The *cease*-fire agreement brought a temporary cease to the conflict.`,

        `Solving complex puzzles can be a great *challenge* for the mind.
The internet is a powerful *channel* for sharing knowledge and connecting people worldwide.
Honesty is a key *characteristic* of a trustworthy person.
In your research paper, remember to *cite* your sources to give credit to the original authors.
*Civil* rights movements have played a crucial role in achieving equality for all people.`,

        `Can you *clarify* your instructions? I'm not sure what you mean.
"To Kill a Mockingbird" is considered a *classic* novel of American literature.
Our vacation plans *coincide* with the local festival, so we'll get to experience it.
The old building was in such disrepair that it was in danger of *collapse*.
My *colleague* and I have been collaborating on this project for several months.`,

        `Students need to *calculate* the total sum of the numbers in this equation.
She is *capable* of handling challenging tasks with ease.
The animals were kept in *captivity* for research purposes.
The children played *carefree* in the park, enjoying the sunny day.
The historical artifact dates back to the 18th *century*.`,

        `The hidden treasure was found in a secret *chamber* deep within the castle.
The Earth follows a *circular* orbit around the Sun.
She tried to *coax* her cat out of hiding with a treat.
The newspaper has a regular *column* for opinion pieces.
People use various forms of language to *communicate* with each other.`,

        `The Olympic Games are a showcase of international *competition*.
The puzzle is not *complete* without all the missing pieces.
The chef had to *concentrate* the sauce to enhance its flavor.
Environmental *concerns* have led to a push for sustainability.
The meeting will *conclude* with a summary of the decisions made.`
      ]
    },
    {
      type: 'completeWord',
      id: 'complete-word-1',
      label: 'Complete Word',
      data: [
        `Characterized by perfect conformity to fact or truth | ACCURATE
To speak or write to someone | ADDRESS
Have the financial means to do something or buy something | AFFORD
Warn or arouse to a sense of danger | ALERT
Carefully observe, investigate, or study something to gain a deeper understanding | ANALYZE
Individuals to whom a person is related by blood or descent | ANCESTOR
Occurring every year | ANNUAL
Clear, obvious, easily seen or understood | APPARENT
Suitable for a particular person, place, or situation | APPROPRIATE
A large stage for open air sports or entertainments | ARENA`,

        `Take into custody | ARREST
Travel up | ASCEND
Give help; be of service | ASSIST
Make an effort | ATTEMPT
Pleasing to the eye or mind | ATTRACTIVE
A state of equilibrium or equipoise; equal distribution of weight | BALANCE
A structure or object that blocks free movement | BARRIER
Something that promotes well-being | BENEFIT
A partiality that prevents objective consideration of an issue or situation | BIAS
A connection that fastens things together | BOND`,

        `Of short duration or distance | BRIEF
Indicates that something is large or takes up a significant part of something | BULK
The ability to perform or produce | CAPACITY
A way to group or classify things that share certain characteristics or attributes | CATEGORY
Put an end to a state or an activity | CEASE
A demanding or stimulating situation | CHALLENGE
A pathway through which information flows | CHANNEL
A distinguishing quality or trait | CHARACTERISTIC
To quote as an authority or example | CITE
Relating to ordinary citizens and their concerns | CIVIL`,

        `To make clear and understandable | CLARIFY
Something that has stood the test of time and remains relevant, valuable, or appreciated across generations | CLASSIC
To occur at the same time or place | COINCIDE
To fall down or give way suddenly | COLLAPSE
An associate or coworker, especially in a professional setting | COLLEAGUE
Make a mathematical computation | CALCULATE
Having ability | CAPABLE
The state of being imprisoned | CAPTIVITY
Without trouble or worry | CAREFREE
A period of 100 years | CENTURY`,

        `A natural or artificial enclosed space | CHAMBER
Having a circular shape | CIRCULAR
Influence or persuade by gentle and persistent urging | COAX
A line of units following one after another | COLUMN
Convey or share information, thoughts, ideas, or feelings with someone else or a group of people| COMMUNICATE
The act of contending with others for rewards or resources | COMPETITION
Having all necessary qualities | COMPLETE
Make denser, stronger, or purer; giving full and undivided attention to something | CONCENTRATE
Something that interests you because it is important | CONCERN
Bring to a close | CONCLUDE`
      ]
    },
    {
      id: 'meanings-2',
      type: 'slides2',
      label: `Word Meanings`,
      commonData: {
        title: `Word Meanings`,
        displayType: 'custom',
        autoPlay: false,
        substep: false
      },
      data: [
        {
          steps: [
            [
              {
                type: 'title',
                text: 'confuse'
              },
              '- mistake one thing for another',
              {
                type: 'html',
                text: `Eg: <br><br>
The similar-sounding words can easily <i><b>confuse</b></i> new language learners.<br><br>
Don't <i><b>confuse</b></i> the two different routes; one leads to the beach, and the other to the forest.`
              }
            ],
            [
              {
                type: 'title',
                text: 'congratulate'
              },
              '- say something to someone that expresses praise',
              {
                type: 'html',
                text: `Eg: <br><br>
We should <i><b>congratulate</b></i> the team on their remarkable victory.<br><br>
He sent a card to <i><b>congratulate</b></i> his friend on their new job.`
              }
            ],
            [
              {
                type: 'title',
                text: 'considerable'
              },
              '- large in number, amount, extent, or degree',
              {
                type: 'html',
                text: `Eg: <br><br>
The project required a <i><b>considerable</b></i> amount of time and resources.<br><br>
The storm caused <i><b>considerable</b></i> damage to the coastal town.`
              }
            ],
            [
              {
                type: 'title',
                text: 'content'
              },
              '- satisfied or showing satisfaction with things as they are',
              {
                type: 'html',
                text: `Eg: <br><br>
After a fulfilling meal, he felt <i><b>content</b></i> and happy.<br><br>
Being <i><b>content</b></i> with what you have can lead to a more peaceful life.`
              }
            ],
            [
              {
                type: 'title',
                text: 'contribute'
              },
              '- give, provide, or supply something',
              {
                type: 'html',
                text: `Eg: <br><br>
Many people volunteered to <i><b>contribute</b></i> food and clothing to the charity.<br><br>
Each team member can <i><b>contribute</b></i> their unique skills to the project.`
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'crafty'
              },
              '- someone who is clever, cunning, or devious, often with an intent to deceive or manipulate others',
              {
                type: 'html',
                text: `Eg: <br><br>
The <i><b>crafty</b></i> fox outwitted the hunters and escaped with the prize.<br><br>
She used her <i><b>crafty</b></i> negotiation skills to secure a better deal.`
              }
            ],
            [
              {
                type: 'title',
                text: 'create'
              },
              '- bring into existence',
              {
                type: 'html',
                text: `Eg: <br><br>
Artists often <i><b>create</b></i> beautiful works of art that inspire and move people.<br><br>
Scientists work tirelessly to <i><b>create</b></i> innovative solutions to global challenges.`
              }
            ],
            [
              {
                type: 'title',
                text: 'demonstrate'
              },
              '- give an exhibition of to an interested audience',
              {
                type: 'html',
                text: `Eg: <br><br>
The inventor will <i><b>demonstrate</b></i> the new technology at the product launch.<br><br>
Teachers often <i><b>demonstrate</b></i> problem-solving techniques to their students.`
              }
            ],
            [
              {
                type: 'title',
                text: 'descend'
              },
              '- move downward and lower, but not necessarily all the way',
              {
                type: 'html',
                text: `Eg: <br><br>
The hikers began to <i><b>descend</b></i> the steep mountain trail as the sun set.<br><br>
The plane will <i><b>descend</b></i> to a lower altitude as it prepares for landing.`
              }
            ],
            [
              {
                type: 'title',
                text: 'desire'
              },
              '- the feeling that accompanies an unsatisfied state',
              {
                type: 'html',
                text: `Eg: <br><br>
His <i><b>desire</b></i> for adventure led him to explore remote places and cultures.<br><br>
The <i><b>desire</b></i> for knowledge drives many to pursue education and learning.`
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'destructive'
              },
              '- causing damage',
              {
                type: 'html',
                text: `Eg: <br><br>
Natural disasters like hurricanes can be highly <i><b>destructive</b></i> to communities.<br><br>
The use of <i><b>destructive</b></i> weapons in warfare has devastating consequences.`
              }
            ],
            [
              {
                type: 'title',
                text: 'develop'
              },
              '- progress or evolve through a process of natural growth',
              {
                type: 'html',
                text: `Eg: <br><br>
Children <i><b>develop</b></i> physically and intellectually as they grow into adulthood.<br><br>
Scientific research helps us <i><b>develop</b></i> new technologies and understanding.`
              }
            ],
            [
              {
                type: 'title',
                text: 'disaster'
              },
              '- an event resulting in great loss and misfortune',
              {
                type: 'html',
                text: `Eg: <br><br>
Earthquakes and tsunamis are natural <i><b>disasters</b></i> that can have widespread devastation.<br><br>
The company faced a financial <i><b>disaster</b></i> due to poor management decisions.`
              }
            ],
            [
              {
                type: 'title',
                text: 'disclose'
              },
              '- expose to view as by removing a cover',
              {
                type: 'html',
                text: `Eg: <br><br>
The government decided to <i><b>disclose</b></i> classified information for public safety.<br><br>
It's essential to <i><b>disclose</b></i> potential conflicts of interest in business dealings.`
              }
            ],
            [
              {
                type: 'title',
                text: 'distract'
              },
              `- draw someone's attention away from something`,
              {
                type: 'html',
                text: `Eg: <br><br>
The noisy construction outside the window would <i><b>distract</b></i> her from her work.<br><br>
It's best not to <i><b>distract</b></i> the driver when they are behind the wheel.`
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'distress'
              },
              '- a state of adversity',
              {
                type: 'html',
                text: `Eg: <br><br>
Providing relief to those in <i><b>distress</b></i> is a humanitarian mission.<br><br>
The loss of a loved one can cause profound emotional <i><b>distress</b></i>.`
              }
            ],
            [
              {
                type: 'title',
                text: 'dusk'
              },
              '- the time of day immediately following sunset',
              {
                type: 'html',
                text: `Eg: <br><br>
The park becomes tranquil and serene at <i><b>dusk</b></i>, as the day transitions into night.<br><br>
Many nocturnal animals become active at <i><b>dusk</b></i>.`
              }
            ],
            [
              {
                type: 'title',
                text: 'eager'
              },
              '- having or showing keen interest or intense desire',
              {
                type: 'html',
                text: `Eg: <br><br>
The students were <i><b>eager</b></i> to learn and participate in the class discussion.<br><br>
The young explorer was <i><b>eager</b></i> to embark on his first adventure.`
              }
            ],
            [
              {
                type: 'title',
                text: 'ease'
              },
              '- freedom from difficulty or hardship or effort',
              {
                type: 'html',
                text: `Eg: <br><br>
Relaxing by the beach provides a sense of <i><b>ease</b></i> and tranquility.<br><br>
A well-organized workspace can enhance productivity and make tasks more <i><b>ease</b></i>.`
              }
            ],
            [
              {
                type: 'title',
                text: 'entertain'
              },
              '- provide amusement for',
              {
                type: 'html',
                text: `Eg: <br><br>
Comedians use humor to <i><b>entertain</b></i> their audiences and make them laugh.<br><br>
The circus aims to <i><b>entertain</b></i> both children and adults with acrobatics and performances.`
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'create'
              },
              '- bring into existence',
              {
                type: 'html',
                text: `Eg: <br><br>
Artists often <i><b>create</b></i> beautiful works of art that inspire and move people.<br><br>
Scientists work tirelessly to <i><b>create</b></i> innovative solutions to global challenges.`
              }
            ],
            [
              {
                type: 'title',
                text: 'demonstrate'
              },
              '- give an exhibition of to an interested audience',
              {
                type: 'html',
                text: `Eg: <br><br>
The inventor will <i><b>demonstrate</b></i> the new technology at the product launch.<br><br>
Teachers often <i><b>demonstrate</b></i> problem-solving techniques to their students.`
              }
            ],
            [
              {
                type: 'title',
                text: 'descend'
              },
              '- move downward and lower, but not necessarily all the way',
              {
                type: 'html',
                text: `Eg: <br><br>
The hikers began to <i><b>descend</b></i> the steep mountain trail as the sun set.<br><br>
The plane will <i><b>descend</b></i> to a lower altitude as it prepares for landing.`
              }
            ],
            [
              {
                type: 'title',
                text: 'desire'
              },
              '- the feeling that accompanies an unsatisfied state',
              {
                type: 'html',
                text: `Eg: <br><br>
His <i><b>desire</b></i> for adventure led him to explore remote places and cultures.<br><br>
The <i><b>desire</b></i> for knowledge drives many to pursue education and learning.`
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'destructive'
              },
              '- causing damage',
              {
                type: 'html',
                text: `Eg: <br><br>
Natural disasters like hurricanes can be highly <i><b>destructive</b></i> to communities.<br><br>
The use of <i><b>destructive</b></i> weapons in warfare has devastating consequences.`
              }
            ],
            [
              {
                type: 'title',
                text: 'develop'
              },
              '- progress or evolve through a process of natural growth',
              {
                type: 'html',
                text: `Eg: <br><br>
Children <i><b>develop</b></i> physically and intellectually as they grow into adulthood.<br><br>
Scientific research helps us <i><b>develop</b></i> new technologies and understanding.`
              }
            ],
            [
              {
                type: 'title',
                text: 'disaster'
              },
              '- an event resulting in great loss and misfortune',
              {
                type: 'html',
                text: `Eg: <br><br>
Earthquakes and tsunamis are natural <i><b>disasters</b></i> that can have widespread devastation.<br><br>
The company faced a financial <i><b>disaster</b></i> due to poor management decisions.`
              }
            ],
            [
              {
                type: 'title',
                text: 'disclose'
              },
              '- expose to view as by removing a cover',
              {
                type: 'html',
                text: `Eg: <br><br>
The government decided to <i><b>disclose</b></i> classified information for public safety.<br><br>
It's essential to <i><b>disclose</b></i> potential conflicts of interest in business dealings.`
              }
            ],
            [
              {
                type: 'title',
                text: 'distract'
              },
              `- draw someone's attention away from something`,
              {
                type: 'html',
                text: `Eg: <br><br>
The noisy construction outside the window would <i><b>distract</b></i> her from her work.<br><br>
It's best not to <i><b>distract</b></i> the driver when they are behind the wheel.`
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'distress'
              },
              '- a state of adversity',
              {
                type: 'html',
                text: `Eg: <br><br>
Providing relief to those in <i><b>distress</b></i> is a humanitarian mission.<br><br>
The loss of a loved one can cause profound emotional <i><b>distress</b></i>.`
              }
            ],
            [
              {
                type: 'title',
                text: 'dusk'
              },
              '- the time of day immediately following sunset',
              {
                type: 'html',
                text: `Eg: <br><br>
The park becomes tranquil and serene at <i><b>dusk</b></i>, as the day transitions into night.<br><br>
Many nocturnal animals become active at <i><b>dusk</b></i>.`
              }
            ],
            [
              {
                type: 'title',
                text: 'eager'
              },
              '- having or showing keen interest or intense desire',
              {
                type: 'html',
                text: `Eg: <br><br>
The students were <i><b>eager</b></i> to learn and participate in the class discussion.<br><br>
The young explorer was <i><b>eager</b></i> to embark on his first adventure.`
              }
            ],
            [
              {
                type: 'title',
                text: 'ease'
              },
              '- freedom from difficulty or hardship or effort',
              {
                type: 'html',
                text: `Eg: <br><br>
Relaxing by the beach provides a sense of <i><b>ease</b></i> and tranquility.<br><br>
A well-organized workspace can enhance productivity and make tasks more <i><b>ease</b></i>.`
              }
            ],
            [
              {
                type: 'title',
                text: 'entertain'
              },
              '- provide amusement for',
              {
                type: 'html',
                text: `Eg: <br><br>
Comedians use humor to <i><b>entertain</b></i> their audiences and make them laugh.<br><br>
The circus aims to <i><b>entertain</b></i> both children and adults with acrobatics and performances.`
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'enlighten'
              },
              '- give someone greater knowledge and understanding about a subject or situation',
              {
                type: 'html',
                text: `Eg: <br><br>
The purpose of education is to <i><b>enlighten</b></i> individuals and expand their horizons.<br><br>
The mentor sought to <i><b>enlighten</b></i> the young artist about the subtleties of their craft.`
              }
            ],
            [
              {
                type: 'title',
                text: 'enthusiasm'
              },
              '- intense and eager enjoyment, interest, or approval',
              {
                type: 'html',
                text: `Eg: <br><br>
The crowd's <i><b>enthusiasm</b></i> for the concert was evident through their cheers and applause.<br><br>
She tackled her work with great <i><b>enthusiasm</b></i> and dedication.`
              }
            ],
            [
              {
                type: 'title',
                text: 'establish'
              },
              '- set up (an organization, system, or set of rules) on a firm or permanent basis',
              {
                type: 'html',
                text: `Eg: <br><br>
The company aims to <i><b>establish</b></i> itself as a leader in the tech industry.<br><br>
Governments work to <i><b>establish</b></i> laws and regulations for the welfare of their citizens.`
              }
            ],
            [
              {
                type: 'title',
                text: 'evaluate'
              },
              '- assess or judge the value, importance, quality, or condition of something',
              {
                type: 'html',
                text: `Eg: <br><br>
Teachers <i><b>evaluate</b></i> their students' progress through various assignments and tests.<br><br>
Before making a decision, it's important to carefully <i><b>evaluate</b></i> all available options.`
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'flexible'
              },
              '- able to bend easily',
              {
                type: 'html',
                text: `Eg: <br><br> 
Yoga practitioners often have <i><b>flexible</b></i> bodies, allowing them to perform various poses with ease.<br><br> 
The gymnast's <i><b>flexible</b></i> movements impressed the judges.`
              }
            ],
            [
              {
                type: 'title',
                text: 'focus'
              },
              '- the concentration of attention or energy on something',
              {
                type: 'html',
                text: `Eg: <br><br> 
To succeed in your studies, you need to maintain a high level of <i><b>focus</b></i> and avoid distractions.<br><br> 
The photographer's <i><b>focus</b></i> was on capturing the perfect shot.`
              }
            ],
            [
              {
                type: 'title',
                text: 'fragile'
              },
              '- easily broken or damaged or destroyed',
              {
                type: 'html',
                text: `Eg: <br><br> 
Handle the glassware with care; it's <i><b>fragile</b></i> and can shatter easily.<br><br> 
The antique vase was beautiful but <i><b>fragile</b></i> and required delicate handling.`
              }
            ],
            [
              {
                type: 'title',
                text: 'frantic'
              },
              '- marked by uncontrolled excitement or emotion',
              {
                type: 'html',
                text: `Eg: <br><br> 
When they realized they were running late for the flight, they became <i><b>frantic</b></i> and rushed to the airport.<br><br> 
The news of the championship win sparked <i><b>frantic</b></i> celebrations in the city.`
              }
            ],
            [
              {
                type: 'title',
                text: 'frequent'
              },
              '- coming at short intervals or habitually',
              {
                type: 'html',
                text: `Eg: <br><br> 
The bus service provides <i><b>frequent</b></i> trips to and from the city center, making commuting convenient.<br><br> 
They had <i><b>frequent</b></i> meetings to discuss project progress.`
              }
            ]
          ]
        },
        {
          steps: [
            [
              {
                type: 'title',
                text: 'frontier'
              },
              '- a wilderness at the edge of a settled area of a country',
              {
                type: 'html',
                text: `Eg: <br><br> 
Explorers once embarked on dangerous journeys to discover the <i><b>frontier</b></i> and expand their knowledge of the world.<br><br> 
The town was located near the <i><b>frontier</b></i>, where the untamed wilderness began.`
              }
            ],
            [
              {
                type: 'title',
                text: 'furious'
              },
              '- marked by extreme anger',
              {
                type: 'html',
                text: `Eg: <br><br> 
His rude remarks left her <i><b>furious</b></i>, and she demanded an apology.<br><br> 
The unfair treatment of the employees led to a <i><b>furious</b></i> protest.`
              }
            ],
            [
              {
                type: 'title',
                text: 'generosity'
              },
              '- the trait of being willing to give your money or time',
              {
                type: 'html',
                text: `Eg: <br><br> 
His <i><b>generosity</b></i> was evident when he donated a substantial amount to the charity.<br><br> 
She is known for her <i><b>generosity</b></i> in helping those in need.`
              }
            ],
            [
              {
                type: 'title',
                text: 'hail'
              },
              '- precipitation of ice pellets',
              {
                type: 'html',
                text: `Eg: <br><br> 
In severe weather, <i><b>hail</b></i> can cause damage to crops and vehicles.<br><br> 
The unexpected <i><b>hail</b></i> storm surprised the residents of the area.`
              }
            ],
            [
              {
                type: 'title',
                text: 'hardship'
              },
              '- something difficult to endure',
              {
                type: 'html',
                text: `Eg: <br><br> 
The family faced financial <i><b>hardship</b></i> during the economic downturn and had to make sacrifices.<br><br> 
Overcoming personal <i><b>hardship</b></i> often leads to personal growth.`
              }
            ]
          ]
        }
      ]
    },
    {
      label: 'Fill in the blanks',
      type: 'matchByDragDrop',
      id: 'drag-drop-2',
      lockAfter: 1,
      commonData: {
        title:
          'Drag the correct option in each blank to form meaningfull sentences.'
      },
      data: [
        `The similar-sounding words can easily *confuse* new language learners.
We should *congratulate* the team on their remarkable victory.
The project required a *considerable* amount of time and resources.
After a fulfilling meal, he felt *content* and happy.
Many people volunteered to *contribute* food and clothing to the charity.`,

        `The *crafty* fox outwitted the hunters and escaped with the prize.
Artists often *create* beautiful works of art that inspire and move people.
The inventor will *demonstrate* the new technology at the product launch.
The hikers began to *descend* the steep mountain trail as the sun set.
His *desire* for adventure led him to explore remote places and cultures.`,

        `Natural disasters like hurricanes can be highly *destructive* to communities.
Children *develop* physically and intellectually as they grow into adulthood.
Earthquakes and tsunamis are natural *disasters* that can have widespread devastation.
The government decided to *disclose* classified information for public safety.
The noisy construction outside the window would *distract* her from her work.`,

        `Providing relief to those in *distress* is a humanitarian mission.
The park becomes tranquil and serene at *dusk*, as the day transitions into night.
The students were *eager* to learn and participate in the class discussion.
Relaxing by the beach provides a sense of *ease* and tranquility.
Comedians use humor to *entertain* their audiences and make them laugh.`,

        `The purpose of education is to *enlighten* individuals and expand their horizons.
The crowd's *enthusiasm* for the concert was evident through their cheers and applause.
The company aims to *establish* itself as a leader in the tech industry.
Teachers *evaluate* their students' progress through various assignments and tests.
Yoga practitioners often have *flexible* bodies, allowing them to perform various poses with ease.`,

        `To succeed in your studies, you need to maintain a high level of *focus* and avoid distractions.
Handle the glassware with care; it's *fragile* and can shatter easily.
When they realized they were running late for the flight, they became *frantic* and rushed to the airport.
The bus service provides *frequent* trips to and from the city center, making commuting convenient.
Explorers once embarked on dangerous journeys to discover the *frontier* and expand their knowledge of the world.`,

        `His rude remarks left her *furious*, and she demanded an apology.
His *generosity* was evident when he donated a substantial amount to the charity.
In severe weather, *hail* can cause damage to crops and vehicles.
The family faced financial *hardship* during the economic downturn and had to make sacrifices.`
      ]
    },
    {
      type: 'completeWord',
      id: 'complete-word-2',
      label: 'Complete Word',
      data: [
        `Mistake one thing for another | CONFUSE
Say something to someone that expresses praise | CONGRATULATE
Large in number, amount, extent, or degree | CONSIDERABLE
Satisfied or showing satisfaction with things as they are | CONTENT
Give, provide, or supply something | CONTRIBUTE
Someone who is clever, cunning, or devious, often with an intent to deceive or manipulate others | CRAFTY
Bring into existence | CREATE
Give an exhibition of to an interested audience | DEMONSTRATE
Move downward and lower, but not necessarily all the way | DESCEND
The feeling that accompanies an unsatisfied state | DESIRE
Causing damage | DESTRUCTIVE`,

        `Progress or evolve through a process of natural growth | DEVELOP
An event resulting in great loss and misfortune | DISASTER
Expose to view as by removing a cover | DISCLOSE
Draw someone's attention away from something | DISTRACT
A state of adversity | DISTRESS
The time of day immediately following sunset | DUSK
Having or showing keen interest or intense desire | EAGER
Freedom from difficulty or hardship or effort | EASE
Provide amusement for | ENTERTAIN
Give someone greater knowledge and understanding about a subject or situation | ENLIGHTEN
Intense and eager enjoyment, interest, or approval | ENTHUSIASM`,

        `Set up (an organization, system, or set of rules) on a firm or permanent basis | ESTABLISH
Assess or judge the value, importance, quality, or condition of something | EVALUATE
Able to bend easily | FLEXIBLE
The concentration of attention or energy on something | FOCUS
Easily broken or damaged or destroyed | FRAGILE
Marked by uncontrolled excitement or emotion | FRANTIC
Coming at short intervals or habitually | FREQUENT
A wilderness at the edge of a settled area of a country | FRONTIER
Marked by extreme anger | FURIOUS
The trait of being willing to give your money or time | GENEROSITY
Precipitation of ice pellets | HAIL
Something difficult to endure | HARDSHIP`
      ]
    }
  ]
};
