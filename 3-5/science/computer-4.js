...
# Mouse
A mouse is an easy to use tool to point and select things. They are widely available with desktop computers. With laptops also, a mouse can be connected through a USB port.
# Touch Screen
Mouse is not available in smartphones and tablets because we can directly click on the screen and select things.
# Keyboard
It is a widely used input device where we can type words and sentences. We use it to type documents, write emails and fill out forms.
Apart from the letters and numbers, we can find a lot of other keys on a keyboard. Backspace and Delete key are used to delete content.
# Arrow Keys
It is used to move the cursor up, down, left and right. It is also widely used in playing games.
# Caps lock
It is a toggleable key that can be turned 'on' or 'off'. If turned 'on', whatever we type, comes in capital letters.
# Shift Key
Most of the keys in the keyboard are used to input two characters. For example, '?' and '/' are present in the same key. So to input the other character, we use the Shift key.
# Print Screen
It is used to save the screen as an image, which can be stored or shared with others.
# Ctrl Key
It is used for giving special instructions to a computer. Some of the common instructions are
...
To search for the occurrence of a particular text in a document, I use *Ctrl + F*.
...
use a computer
see what is happening on a computer
get an output`
          },
          {
          ...
          {
            qText:
              'If you want to type every character in capital letters, which key do you use?',
            options:
...
# Internet Browsers
They allow us to connect to the internet. Google Chrome, Mozilla Firefox, and Safari are the most commonly used browsers.
# Google
It is the most famous search engine that helps to search on the internet. For example, if you want to know more about a 'tiger', just google it. You will find a lot of information about the tiger.
# Online Meeting App
People can conduct online meetings using software like Zoom, Skype or Google Meet. Multiple people can connect at the same time. We can talk through a microphone. We can show our faces through a web camera. We can also share our screens.
# Social Media
It helps us to connect with our friends and also allows us to follow the activities of celebrities and other people. Facebook, Instagram, and Twitter are the famous social media applications.
# Messaging App
