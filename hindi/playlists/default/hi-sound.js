export default {
  label: 'ध्वनि',
  id: 'hi-sound',
  img: 'sound',
  list: [
    {
      type: 'slides',
      id: 'animals',
      label: 'जानवर',
      commonData: {
        audio: 'hi/hi-animals-1.mp3'
      },
      data: [
        {
          audioOffset: 0,
          text: `cat| बिल्ली
          dog| कुत्ता
          goat|बकरी
          sheep|भेड़
          horse|घोड़ा
          pig|सूअर
          elephant|हाथी
          lion|शेर
          monkey|बंदर
          tiger|बाघ`
        },
        {
          audioOffset: 20,
          text: `hippo|जलहस्ती
          rhino| गैंडा
          fox|लोमड़ी
          crocodile|मगरमच्छ
          zebra|ज़ेबरा
          bear|भालू
          camel|ऊंट
          shark| हाँगर
          dragonfly|व्याध-पतंग
          fish|मछली`
        },
        {
          audioOffset: 40,
          text: `frog| मेंढक
          crab|केकड़ा
          snake|साँप
          squirrel|गिलहरी
          tortoise|कछुआ
          rat|चूहा
          winged-termites|दीमक`
        },
        {
          audioOffset: 54,
          text: `ant|चींटी
          fly|मक्खी
          bat|चमगादड़
          buffalo|भैंस
          butterfly|तितली
          cow|गाय
          donkey|गधा`
        }
      ]
    },
    {
      type: 'slides',
      id: 'birds',
      label: 'चिड़िया',
      commonData: {
        audio: 'hi/hi-birds-1.mp3'
      },
      data: [
        {
          audioOffset: 0,
          text: `crow|कौआ
          dove| कबूतर
          duck| बतख़
          hen| मुर्गी
          parrot| तोता
          peacock| मोर
          sparrow|गौरैया
          chick|चूज़ा`
        },
        {
          audioOffset: 16,
          text: `crane| सारस
          eagle| चील
          flamingo| मराल
          kingfisher| नीलकंठ
          myna| मैना
          ostrich| शुतुरमुर्ग 
          owl| उल्लू
          rooster| मुर्गा
          vulture | गिद्ध`
        }
      ]
    },
    {
      type: 'slides',
      id: 'fruits',
      label: 'फल',
      commonData: {
        audio: 'hi/hi-fruits-1.mp3'
      },
      data: [
        {
          audioOffset: 0,
          text: `apple| सेब
          grapes| अंगूर
          banana| केला
          strawberry| झरबेर
          guava| अमरूद
          jackfruit| कटहल
          mango| आम
          orange| संतरा
          papaya| पपीता
          pineapple| अनानास`
        },
        {
          audioOffset: 20,
          text: `pomegranate| अनार
          watermelon| तरबूज
          fig| अंजीर
          custard-apple| शरीफा
          plum| आलूबुखारा
          dates| खजूर
          muskmelon| खरबूज
          star-fruit| कमरख
          dragon-fruit| पिताया 
          passion-fruit| कृष्णा फल`
        },
        {
          audioOffset: 40,
          text: `sugarcane|ईख
          peach|सतालू
          sweet-potato|शकरकंद
          jamun|जामुन
          pear|नाशपाती
          water-chesnut|सिंघाड़ा
          jujube| बेर
          sapotta| चीकू
          blueberry |नीलबदरी`
        }
      ]
    },
    {
      type: 'slides',
      id: 'vegetables',
      label: 'सब्जि',
      commonData: {
        audio: 'hi/hi-vegetables-1.mp3'
      },
      data: [
        {
          audioOffset: 0,
          text: `carrot| गाजर
          beetroot| चुकंदर
          onion| प्याज़
          tomato| टमाटर
          potato| आलू
          cabbage| पत्तागोभी
          cauliflower| फूलगोभी
          corn| भुट्टा
          cucumber| खीरा
          garlic| लहसुन`
        },
        {
          audioOffset: 20,
          text: `ginger| अदरक
          pumpkin| कद्दू
          almond| बादाम
          beans| फलि
          bitter-gourd| करेला
          brinjal| बैंगन
          capsicum| शिमला मिर्च
          cashewnut| काजू
          chilli| हरी मिर्च
          coconut| नारियल`
        }
      ]
    },
    {
      type: 'slides',
      id: 'house',
      label: 'मकान',
      commonData: {
        audio: 'hi/hi-house-dress-1.mp3'
      },
      data: [
        {
          audioOffset: 0,
          text: `man| आदमी
          woman| औरत
          boy| लड़का
          girl| लड़की
          house| मकान
          grandfather| दादा
          table| मेज
          bed| बिस्तर
          candle| मोमबत्ती
          chair| कुर्सी`
        },
        {
          audioOffset: 20,
          text: `shelf| अलमारी
          baby| बच्चा
          ball| गेंद
          glasses| ऐनक
          clock| दीवार घड़ी
          watch| घडी
          flower-pot|गमला
          umbrella|छतरी
          door| दरवाजा
          broom| झाड़ू`
        },
        {
          audioOffset: 40,
          text: `window|खिड़की
          bucket|बाल्टी
          soap|साबुन
          pillow|तकिया
          basket|टोकरी
          dustbin|कचरे का डिब्बा`
        }
      ]
    },
    {
      type: 'slides',
      id: 'dress',
      label: 'पोशाक',
      data: {
        audio: 'hi/hi-house-dress-1.mp3',
        audioOffset: 52,
        text: `shoe|जूता
        sock|मौज़ा
        pyjama|पजामा
        gloves|दस्ताने
        scarf|दुपट्टा
        shirt|कमीज
        hat|टोपी
        trousers|पतलून`
      }
    },
    {
      type: 'slides',
      id: 'kitchen',
      label: 'रसोईघर',
      commonData: {
        audio: 'hi/hi-kitchen-body-1.mp3'
      },
      data: [
        {
          audioOffset: 0,
          text: `stove| चूल्हा
          fork| कांटा
          knife| छुरी
          pan| कड़ाही
          spoon | चम्मच 
          pot| मटका
          butter| मक्खन
          plate|थाली
          tap|नल
          lid|ढक्कन`
        },
        {
          audioOffset: 20,
          text: `cheese|पनीर
          lollipop|टिकीया
          sugar|चीनी
          bowl|कटोरा
          meat|मांस
          salt|नमक`
        },
        {
          audioOffset: 32,
          text: ` sweets|मिठाइयाँ
          bread|रोटी
          juice|रस
          flour|आटा
          lentils|दाल
          rice|चावल`
        }
      ]
    },
    {
      type: 'slides',
      id: 'bodyParts',
      label: 'शरीर के अंग',
      data: {
        audio: 'hi/hi-kitchen-body-1.mp3',
        audioOffset: 44,
        text: `ear| कान
        eye| आँख
        foot| पैर
        hair| बाल
        hand|  हाथ
        mouth| मुँह
        nose| नाक
        tooth| दांत`
      }
    },
    {
      type: 'slides',
      id: 'nature',
      label: 'प्रकृति',
      commonData: {
        audio: 'hi/hi-nature-other-1.mp3'
      },
      data: [
        {
          audioOffset: 0,
          text: `sun| सूर्य
          moon| चांद
          rain| बारिश
          star| तारा
          milk| दूध
          bird|पक्षी`
        },
        {
          audioOffset: 12,
          text: `fruits|फल
          leaf|पत्ता
          world|दुनिया
          nest|घोंसला
          water-drop|ओस
          waterfall|झरना
          rainbow|इंद्रधनुष`
        }
      ]
    },
    {
      type: 'slides',
      id: 'other-images',
      label: 'अन्य छवियां',
      commonData: {
        audio: 'hi/hi-nature-other-1.mp3'
      },
      data: [
        {
          audioOffset: 26,
          text: `ring| अंगूठी
          brush| कूंचा
          bone| हड्डी
          bell| घंटी
          conch| शंख
          drums| ढोल
          five| पांच`
        },
        {
          audioOffset: 40,
          text: `food| खाना
          king| राजा
          kite| पतंग
          ladder| सीढ़ी
          ladoo| लड्डू
          rishi| ऋषि मुनि
          wheel| चक्का
          whistle| सीटी`
        }
      ]
    },
    {
      type: 'rightOne',
      label: 'चित्र चुनें',
      lockAfter: 3,
      id: 'pick-word',
      commonData: {
        title: 'ध्वनि सुनकर चित्र का चयन करें',
        type: 'image'
      },
      data: [
        {
          audio: 'hi/hi-animals-1.mp3',
          words:
            'cat, dog, goat, sheep, horse, pig, elephant, lion, monkey, tiger'
        },
        {
          audio: 'hi/hi-animals-1.mp3',
          audioOffset: 20,
          words:
            'hippo, rhino, fox, crocodile, zebra, bear, camel, shark, dragonfly, fish'
        },
        {
          audio: 'hi/hi-animals-1.mp3',
          audioOffset: 40,
          words: 'frog, crab, snake, squirrel, tortoise, rat, winged-termites'
        },
        {
          audio: 'hi/hi-animals-1.mp3',
          audioOffset: 54,
          words: 'ant, fly, bat, buffalo, butterfly, cow, donkey'
        },
        {
          audio: 'hi/hi-birds-1.mp3',
          words:
            'crow, dove, duck, hen, parrot, peacock, sparrow, chick'
        },
        {
          audio: 'hi/hi-birds-1.mp3',
          audioOffset: 16,
          words: 'crane, eagle, flamingo, kingfisher, myna, ostrich, owl, rooster, vulture'
        },
        {
          audio: 'hi/hi-fruits-1.mp3',
          audioOffset: 0,
          words: 'apple, grapes, banana, strawberry, guava, jackfruit, mango, orange, papaya, pineapple'
        },
        {
          audio: 'hi/hi-fruits-1.mp3',
          audioOffset: 20,
          words: 'pomegranate, watermelon, fig, custard-apple, plum, dates, muskmelon, star-fruit, dragon-fruit, passion-fruit'
        },
        {
          audio: 'hi/hi-fruits-1.mp3',
          audioOffset: 40,
          words: 'sugarcane, peach, sweet-potato, jamun, pear, water-chestnut, jujube, sapotta, blueberry'
        },
        {
          audio: 'hi/hi-vegetables-1.mp3',
          audioOffset: 0,
          words:
            'carrot, beetroot, onion, tomato, potato, cabbage, cauliflower, corn, cucumber, garlic'
        },
        {
          audio: 'hi/hi-vegetables-1.mp3',
          audioOffset: 20,
          words:
            'ginger, pumpkin, almond, beans, bitter-gourd, brinjal, capsicum, cashewnut, chilli, coconut'
        },
        {
          audio: 'hi/hi-house-dress-1.mp3',
          audioOffset: 0,
          words:
            'man, woman, boy, girl, house, grandfather, table, bed, candle, chair'
        },
        {
          audio: 'hi/hi-house-dress-1.mp3',
          audioOffset: 20,
          words:
            'shelf, baby, ball, glasses, clock, watch, flower-pot, umbrella, door, broom'
        },
        {
          audio: 'hi/hi-house-dress-1.mp3',
          audioOffset: 40,
          words:
            'window, bucket, soap, pillow, basket, dustbin'
        },
        {
          audio: 'hi/hi-house-dress-1.mp3',
          audioOffset: 52,
          words: 'shoe, sock, pyjama, gloves, scarf, shirt, hat, trousers'
        },
        {
          audio: 'hi/hi-kitchen-body-1.mp3',
          audioOffset: 0,
          words: 'stove, fork, knife, pan, spoon, pot, butter, plate, tap, lid'
        },
        {
          audio: 'hi/hi-kitchen-body-1.mp3',
          audioOffset: 20,
          words: 'cheese, lollipop, sugar, bowl, meat, salt'
        },
        {
          audio: 'hi/hi-kitchen-body-1.mp3',
          audioOffset: 32,
          words: 'sweets, bread, juice, flour, lentils, rice'
        },
        {
          audio: 'hi/hi-kitchen-body-1.mp3',
          audioOffset: 44,
          words: 'ear, eye, foot, hair, hand, mouth, nose, tooth'
        },
        {
          audio: 'hi/hi-nature-other-1.mp3',
          audioOffset: 0,
          words: 'sun, moon, rain, star, milk, bird'
        },
        {
          audio: 'hi/hi-nature-other-1.mp3',
          audioOffset: 12,
          words: 'fruits, leaf, world, nest, water-drop, waterfall, rainbow'
        },
        {
          audio: 'hi/hi-nature-other-1.mp3',
          audioOffset: 26,
          words:
            'ring, brush, bone, bell, conch, drums, five'
        },
        {
          audio: 'hi/hi-nature-other-1.mp3',
          audioOffset: 40,
          words:
            'food, king, kite, ladder, ladoo, rishi, wheel, whistle'
        }
      ]
    }
  ]
};
